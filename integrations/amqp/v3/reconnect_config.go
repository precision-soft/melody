...
func nextReconnectBackoff(config ReconnectConfig, current time.Duration) time.Duration {
    ...
    /* clamp in float64 before the time.Duration conversion: a large BackoffFactor or MaxBackoff can push the product past the int64 nanosecond range, where the conversion wraps negative, slips past a later `next > maxBackoff` check and drives time.After into a no-delay reconnect storm */
...
/* a subscription resets the backoff only when it lived at least the initial backoff, so one that loses its channel immediately keeps backing off instead of storming the broker. The threshold is the clamped initial backoff, as at every seed and reset site, so a zero InitialBackoff still compares against the default. */
