...
- `provider.go` — `NewClient` refuses an empty access key or secret key instead of silently downgrading to anonymous access. minio treats either empty credential as a request for the anonymous signer rather than an error, so a missing credential env var booted cleanly, served reads off any public bucket policy, and failed writes much later with a bare `AccessDenied` that pointed nowhere near the cause. The refusal names the endpoint and which credential is missing; a caller who genuinely wants anonymous access holds a decision, not an accident, and can build the minio client directly
