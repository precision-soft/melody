...
- `provider.go` — **Behavioural change**: `NewClient` refuses an empty access key or secret key instead of silently downgrading to anonymous access. minio treats either empty credential as a request for the anonymous signer rather than an error, so a missing credential env var booted cleanly, served reads off any public bucket policy, and failed writes much later with a bare `AccessDenied` that pointed nowhere near the cause. The refusal names the endpoint and which credential is missing; a caller who genuinely wants anonymous access holds a decision, not an accident, and can build the minio client directly
- `provider.go` — `EnsureBucket` no longer fails the losing replica of a bucket-creation race. The exists-then-create pair is not atomic: two replicas booting together both read "absent", both create, and the loser's `BucketAlreadyOwnedByYou`/`BucketAlreadyExists` refusal named a bucket that now exists — exactly the state the function promises. On those two conflict codes the bucket's existence is re-checked, and only a probe that confirms the bucket is this caller's to use reads as success; a name owned by another account keeps failing, because there the re-check still answers absent
- `service_resolver.go` — `RegisterStorageService` validates the client and the bucket at registration instead of deferring to the provider closure: `NewStorage` panics on a nil client or an empty bucket, and inside the closure that panic fired at the first resolution — possibly on a request path — instead of at boot, where every other integration reports an unusable configuration
- `storage.go` — the size check's end-of-body probe surfaces a transport failure instead of folding it into a clean end. A body that broke exactly at the declared boundary answered `io.EOF`, fabricating an exact-match verdict for a body whose length past the boundary is unknown — the one question the reader exists to answer
- `storage.go` — every storage error now names the bucket beside the key, so a process wired over more than one bucket can attribute a failure; the field was on the struct and in no error context
