...
| `Timeout`         | bounds one run of this entry under the **in-process runner** (`melody:cron:run`); the generated manifests ignore it. Zero takes the runner default of **1 hour**; a negative value opts out of the deadline entirely. A command cancelled by the deadline is reported wrapping `ErrCommandTimeout`; one that never returns is abandoned five seconds later, reported, and its container scope released — otherwise a command wedged on a deadline-less read would hold a goroutine and a scope per matching minute for the life of the process |
