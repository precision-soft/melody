...
/* Schedule keeps the registration order: the generator renders the entries in that order and the runner parses them in it, so a registry that reordered them would emit a crontab the caller cannot predict. */
...
/* the copy reaches past the list: ScheduledCommand and EntryConfig are exported structs with exported fields, so a caller writing through a returned pointer would rewrite the registration itself, the mutation Schedule's copy prevents. */
