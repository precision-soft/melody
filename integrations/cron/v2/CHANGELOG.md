...
- a scheduled command runs under a deadline, one hour by default and overridable per entry, so a command wedged on a call that never returns no longer leaks a goroutine and an unreleased container scope on every matching minute. A negative value is the explicit opt-out. Overlap between runs is unchanged and remains the exclusivity wrapper's job
