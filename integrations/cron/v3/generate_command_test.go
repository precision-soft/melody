...
func TestRunPruneDeletesConfiguredK8sManifest(t *testing.T) {
    dir := t.TempDir()
    target := filepath.Join(dir, "reports.yaml")
    commands := []clicontract.Command{
        newFakeCommandWithConfig("reports:daily", &EntryConfig{
            Schedule: &Schedule{Minute: "0", Hour: "3"},
    ...
    args := []string{"--out", filepath.Join(dir, "default.yaml"), "--template", "k8s", "--image", "registry.example/app:1"}
    if _, err := runGenerateCommand(t, commands, args); nil != err {
        t.Fatal(err)
    }
    if _, err := runGenerateCommand(t, commands, append(args, "--prune")); nil != err {
        t.Fatal(err)
    }
    if _, err := os.Lstat(target); false == os.IsNotExist(err) {
