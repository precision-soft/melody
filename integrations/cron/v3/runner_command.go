...
Due commands run concurrently, each in its own goroutine, the way crontab starts an independent process per entry: one slow job delays neither the commands sharing its minute nor the scheduler loop, and an entry that runs longer than its own interval overlaps itself — wrap the command in lock.NewExclusiveCommand to serialize successive runs. Each run is bounded by a deadline, defaultCommandTimeout unless EntryConfig.Timeout says otherwise, because nothing in the runtime context would ever end a command wedged on a deadline-less read; a command abandoned past that deadline is reported and stops counting towards the shutdown wait, since waiting on it would never end either. Wall-clock jumps follow the vixie-cron virtual-time algorithm, documented on reconcileWallClock, so a schedule pinned inside a daylight-saving gap still runs exactly once. Multi-instance safety is left to composition — wrap each command in lock.NewExclusiveCommand, or gate the whole runner behind a lock.LeaderGate, before handing the commands in. */
