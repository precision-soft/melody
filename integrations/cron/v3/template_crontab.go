...
/* CrontabOwnershipMarker opens the ownership line every destination this generator's builtin templates render carries — the two crontab dialects in their header block, the k8s dialect as a leading YAML comment — so a later --prune can tell a file this generator wrote from one the operator put in the same directory. It names the command rather than saying "generated", because "GENERATED FILE" is what every generator writes and proves nothing about which one. The line a run writes is this prefix followed by " for " and the name the application runs under — its cli name — because the command alone proves the writing COMMAND and not the writing APPLICATION: two melody applications sharing an output directory rendered identical lines, and the sweep of one emptied the destinations of the other. The line is shared across the builtin dialects, so it proves the application and the command, not the dialect. Only a template the generator has handed the application's name renders the whole line; the builtin singletons and the package-level Render write the bare prefix, which the sweep of a named application never matches, so a file written that way is recognisable and left alone. */
