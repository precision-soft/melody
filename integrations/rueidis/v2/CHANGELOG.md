...
## [v2.2.0] - 2026-08-18 - Wiring-Time Credential Redaction, Cluster-Wide Cache Sweeps and Bounded Rate-Limiter Calls
...
- `provider.go` — the redis password parameter was marked secret only inside `Open`, which runs at the first dial, so a process that never resolves the client — a console run, `debug:parameters` above all — printed the live credential unmasked in both the table and the json document, while the database password in the same output was masked. `Provider.SecretParameterNames` now names the credential the way the bunorm providers do, and the new `MarkSecretParameters` arms the framework's redaction at wiring time through the tolerant configuration door — a resolver without a configuration service leaves the marking undone instead of failing; the marking inside `Open` stays as the cover for a wiring that never calls it
...
[Unreleased]: https://github.com/precision-soft/melody/compare/integrations/rueidis/v2.2.0...HEAD

[v2.2.0]: https://github.com/precision-soft/melody/compare/integrations/rueidis/v2.1.0...integrations/rueidis/v2.2.0
