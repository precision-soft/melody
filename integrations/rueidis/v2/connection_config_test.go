...
func TestNewConnectionConfig_StoresFields(t *testing.T) {
    connectionConfig := NewConnectionConfig("localhost:6379", "app-user", "super-secret")

    if "localhost:6379" != connectionConfig.Address {
        t.Fatalf("address: expected %q, got %q", "localhost:6379", connectionConfig.Address)
    }

    if "app-user" != connectionConfig.User {
        t.Fatalf("user: expected %q, got %q", "app-user", connectionConfig.User)
    }

    if "super-secret" != connectionConfig.Password {
        t.Fatalf("password: expected %q, got %q", "super-secret", connectionConfig.Password)
    }
}

func TestConnectionConfig_SafeContext_ElidesPassword(t *testing.T) {
    connectionConfig := NewConnectionConfig("localhost:6379", "app-user", "super-secret")
