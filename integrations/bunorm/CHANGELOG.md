...
- `v3/audit/` — a pointer-typed encrypted column (`*EncryptedString`/`*EncryptedDeterministicString`, the natural shape for a nullable encrypted field) is now redacted the same as the value-typed form. `isRedactedField` compared the field type by exact value type only, so a pointer field did not match and — because the encrypted types define no `MarshalJSON` — its plaintext was serialized into the audit `changes` column (and the dead-letter log). The matcher now dereferences pointer types before comparing.
