...
- the transient classifier reads the identity a server refusal carries before it reads the message, and the caller's own context before it classifies at all. The server quotes the operand in its message — a database named `timeout`, a user named `eof` — so a permanent refusal spelled around a transient marker was retried for the whole budget and then filed under `ErrDatabaseUnreachable`, the one class `ReadWriteSplitter.Reader` serves the primary for, while the same refusal spelled around another name was terminal at once; measured against a live server, both. A refusal carrying a SQLSTATE is now classified on its class: `08` (connection exception), `53` (insufficient resources) and `57` (operator intervention) are the server saying it cannot take the connection now and stay transient — an outage such as `08006` or a `57P03` whose message carried no marker used to be read as terminal — and every other class is the server saying no. A caller whose context was already done — a deadline passed before the attempt — used to have its refusal classified transient by the deadline's own timeout: filed as unreachable on the retry-less door, and on the retrying one retried once for nothing and reported as a retry the caller cancelled, two warnings for a caller that had stopped before the first attempt; the caller's context is read first and such a refusal is the caller's clean stop, whichever class it wears. **Behavioural change**: a refusal the server gave by name is never retried and never falls to the primary — see the framework's `UPGRADE.md`
...
- an open failure the transient classifier admitted — and the retry budget could not get past, or the retry-less open met — is filed under `bunorm.ErrDatabaseUnreachable`, the one class a `ReadWriteSplitter` absorbs by reading from the primary; a refusal given by name — an empty parameter, a password the server refused, a database that does not exist — is not, so a misconfigured replica is refused instead of served from the primary in silence. The exception keeps its message and context and the driver failure stays reachable under the class
- an empty database name or user is refused by name before the driver sees it. `pgdriver.WithDatabase` and `pgdriver.WithUser` panic on an empty string, so a connection parameter left empty reached the caller as a panic out of the open rather than as the refusal every other open failure is. An empty host is refused by name as well, for the opposite reason: the driver does not panic on it, and the address `:port` it makes is the local system to a dialer, so a host left unset connected the application to whatever listened on that port on its own machine — with the configured credentials, under the insecure posture — instead of failing. An empty password stays a legitimate value. **Behavioural change**: a configuration whose host was left unset used to connect to the local system and now fails the open by name — see the framework's `UPGRADE.md`.
