...
/* OpenContext observes caller cancellation during retries, hooks and the boot ping. The PostgreSQL dialect performs no construction-time server query. Nil context uses context.Background; nil logger uses the emergency logger. */
