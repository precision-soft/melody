...
`Close` ends every pool the registry memoized, then WAITS for the opens that were still in flight when it published its refusal — and it ENDS them: the refusal cancels the registry's own open context, which every provider of this repository honours in its retry sleeps and its dials, so an open in flight returns instead of dialling on, an open that returns ends its own freshly opened database against the closed flag, and nothing leaks. `Close` used to leave them running under the registry's context, each further attempt routing bun's diagnostics back onto the logger the teardown had just handed back, and to return while a dial was still in the air — a process exiting on that answer left the connection outstanding, its server-side session to be reaped by a timeout rather than ended. The wait is unbounded on `Close` and ends with the caller's deadline on `CloseWithContext`, which is the door the framework container's ordered teardown prefers: a dial a driver does not cut on cancellation ends when the driver gives up, which can outlive a teardown, so the answer then names the opens it stopped waiting for rather than having ended them.
