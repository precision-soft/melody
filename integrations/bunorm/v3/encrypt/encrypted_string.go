...
/* UnmarshalJSON accepts plaintext strings, refuses the redaction placeholder, and leaves the value unchanged on null. */
...
func unmarshalEncryptedJson[T ~string](instance *T, data []byte) error {
    ...
    if unmarshalErr := json.Unmarshal(data, &decoded); nil != unmarshalErr {
        return exception.NewError("encrypted column json value is not a string", map[string]any{"type": fmt.Sprintf("%T", *instance)}, unmarshalErr)
    }
    ...
    if redactedPlaceholder == decoded {
        return exception.NewError(
            ...
            map[string]any{"type": fmt.Sprintf("%T", *instance)},
            nil,
        )
    }
