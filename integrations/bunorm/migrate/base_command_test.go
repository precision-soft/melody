...
/* TestNewMigrator_SqlMigrationExecFailureReachesTheCaller pins the verdict of the SQL migration path against the bun version this module requires: melody builds the migrator and prints the result, so a swallowed exec failure would be a green deploy over an unchanged schema with the migration marked applied forever. bun v1.2.16 lost the failure in its deferred conn.Close, so a bump that reintroduces that fails here. */
