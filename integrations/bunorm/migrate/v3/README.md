...
`db:create` finishes bun's write ATOMICALLY. Bun creates the file with a single `os.WriteFile` straight onto the destination, so a crash, a full disk or a killed process partway through leaves a truncated Go file in the migrations directory — one that does not compile, under a timestamped name whose place in the sequence looks entirely legitimate. Taking the write away from bun is not possible, since the directory it writes into is unexported; what the command does instead is write the same content into a temporary neighbour, fsync it, rename it over the destination and fsync the DIRECTORY, so a command that reports success has left a whole and durable file behind. A crash before that point leaves bun's partial file — and no success report, which is the answer a failed create has always given.
