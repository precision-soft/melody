...
`db:create` finishes bun's write ATOMICALLY. Bun creates the file with a single `os.WriteFile` straight onto the destination, so a crash, a full disk or a killed process partway through leaves a truncated Go file in the migrations directory — one that does not compile, under a timestamped name whose place in the sequence looks entirely legitimate. Taking the write away from bun is not possible, since the directory it writes into is unexported; what the command does instead is write the same content into a temporary neighbour named `.melody-migrate-*.tmp`, which fits one path component whatever the migration's name, fsync it, rename it over the destination and fsync the DIRECTORY, so a command that reports success has left a whole and durable file behind. A failure before the rename removes that temporary neighbour and nothing else; a crash before that point leaves bun's partial file — and no success report, which is the answer a failed create has always given. The rewrite keeps the mode the destination carries — bun's 0644 as narrowed by the process umask — rather than stamping 0644 over it. When the rename landed and only the directory fsync failed, the command succeeds with a warning naming the file and that its entry may not survive a crash: a failure verdict would send the operator to run it again and create a second migration beside a whole first one. The name itself is held to `^[0-9a-z_-]+$` before it reaches bun, so a path separator or a parent reference never touches the filesystem.
