...
`db:create` finalizes Bun's generated file by writing the returned content to a temporary neighbour, syncing and closing it, preserving the destination permissions, renaming it over the destination, and syncing the directory. The temporary name stays short even when the migration filename reaches the filesystem's component limit. Names must match `^[0-9a-z_-]+$` before the generator runs.
