...
- **Behavioural change**: `db:migrate` and `db:rollback` hand their `--no-color` and `--format` posture to the migrations through the context the migrator passes them, and install it as the process-wide fallback only for the length of the run, putting back what was there on the way out. A migration that passes its context on to `RunQueries` — the generated skeleton does — prints under the command that ran it and under nothing else; one that drops the context sees the fallback only while the command runs, where it used to see the last command's writer for the life of the process. `SetDefaultRunnerOption` stays the door of a host that runs migrations on its own.

### Fixed

- a command that PANICS renders a failure document instead of a success one. The json envelope is the machine contract a deploy pipeline reads, and every one of the six commands rendered it from a deferred read of the named return: a panic never reaches the assignment, so the render saw a nil error, skipped the error branch and wrote a complete envelope carrying `"error":null` beside every message the run had accumulated before it fell over — indistinguishable, to anything parsing stdout, from a clean run that applied them. The framework's cli boundary makes this repair for this reason and says so, but it sits outside the command's own defer, and what it adds is a text banner rather than a second document. The commands now defer to one shared door that takes the recovered value as a parameter, because `recover()` answers only when the deferred function calls it directly; the panic is re-raised unchanged once the document says what happened, so the exit path, the status code and the journal record are all exactly what they were. A run that had already failed keeps its own failure as the verdict, and a panic value that is not an error reaches the document as the value it is rather than as a bare type name
- the per-query runner posture is the run's own, not a process value. `RunQueries` read one process-wide default that every migrate command overwrote from its parsed flags, so two commands dispatched concurrently printed under whichever had installed its writer last — measured, a text run's per-query lines went into a `--format=json` run's discarded writer and the text run rendered nothing — and the writer of a finished command stayed installed for whatever ran `RunQueries` next. The option now travels on the context, and `RunQueries` reads it before the process default.
- `meta.arguments` in the json envelope names the positional arguments the command ran on. The envelope was built without them, so every command answered an empty list — `db:create` included, whose one argument is the migration the document reports on.
...
[Unreleased]: https://github.com/precision-soft/melody/compare/integrations/bunorm/migrate/v1.3.0...HEAD

[v1.3.0]: https://github.com/precision-soft/melody/compare/integrations/bunorm/migrate/v1.2.0...integrations/bunorm/migrate/v1.3.0
