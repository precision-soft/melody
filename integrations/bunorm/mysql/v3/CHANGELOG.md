...
- an empty host is refused by name before the driver configuration is built. The address `:port` the driver made from it is the local system to a dialer, so a host left unset connected the application to whatever listened on that port on its own machine — with the configured credentials, the dialect handshake included — instead of failing; the database and the user are left to the server, which refuses an empty one by name, and an empty password stays a legitimate value
- a bare IPv6 literal as the host dials the host it names. The dial address was joined as `host:port`, so `::1` became `::1:3306`, which go-sql-driver re-joins into `[::1:3306]:3306` — a host that does not exist, dialled through the whole retry budget under "database connection failed after max retry attempts" with nothing naming the malformed address; only the bracketed spelling `[::1]` worked. A host carrying a colon is bracketed unless it already is; a host name and an IPv4 literal are joined as before
