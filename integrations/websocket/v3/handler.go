...
/* closeConnection closes the transport and waits at most closeHandshakeGrace for the read loop, keeping the request scope alive for callbacks that finish within that grace. A callback still running afterward may encounter a closed scope; dispatchOnMessage recovers and logs that failure, and its message is lost.

   An active callback prevents the read loop from receiving a close handshake, so CloseNow closes its transport immediately. Otherwise a graceful close runs asynchronously. This shutdown grace is independent of the longer liveness allowance for callbacks on an open connection. */
