...
- `handler.go` — **Behavioural change**: a connection against a hub that has already shut down is refused before the upgrade instead of upgraded and closed instantly. `Subscribe` on a shut-down hub hands back a subscriber whose channel is already closed, which the event loop read as an ordinary end of stream, so a client connecting during the shutdown drain got a `101` and an immediate close with only a generic "read loop ended" debug line — indistinguishable from a peer that walked away. The refusal reads the new `ServerSentEventHub.IsClosed`; a hub that shuts down during the connect window is named at debug before the connection closes
