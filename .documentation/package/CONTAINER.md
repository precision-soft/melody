...
- When the container itself closes, everything still standing in its maps joins the teardown, installed overrides included — the container's lifetime is the process's, so ownership converges on it and there is no opt-in. A scope is deliberately the opposite: it closes what it built and nothing else, and an outside override joins its teardown only through `ClosedWithScope()` (see the scope section below).
