...
- Marking a listener required, with `MarkListenerRequired(registration)`, declares it one that must not be skipped. If another listener stops propagation while a required listener behind it has not run, the dispatch returns an [`event.RequiredListenerSkippedError`](../../event/required_listener_skipped_error.go) instead of completing quietly.
...
- **The dispatch stops at the first failing listener.** The listeners behind it do not run, and the error is returned alongside the partially dispatched event; the caller decides the policy for a partial dispatch. The http kernel fails the request closed for it.
