...
The comparison is a plain prefix test against the request path **as it arrived** — before the strip prefix is removed and before the path is folded — and it is the same test [`NewPathPrefixMatcher`](../../security/matcher.go) makes, so one spelling written in a firewall rule and here selects the same requests. There is one second consultation: a request for the **mount root** resolves to the configured index file, and the list is asked again for that resolved spelling — the strip prefix joined to `/<indexFile>`, so post-strip and post-fold — because an exclusion naming the index file must fire for the page the root actually serves. Two consequences follow from that. An entry must begin with `/`, because a request path always does, and an empty entry would prefix-match every path and switch the file server off wholesale; validation rejects both at boot (`static excluded path must begin with a slash`, `static excluded path may not be empty`), so a stray comma fails the boot instead of
silently taking static serving out of service. And since an entry is a prefix rather than a path element, `/admin` excludes `/administration` too.
...
* A credentialed configuration that decides origins through `AllowOriginFunc` is accepted; one with neither an origin list nor a function still panics on the defaulted wildcard, and one whose list is present but empty panics as well, which are the combinations the guard exists to prevent. See [`NewService`](../../http/cors/service.go).
* **The static file server consults its excluded-path list against the index file the mount root resolves to**, as well as against the path the client sent. `/` — and every spelling that folds onto it — resolves to the configured index file only after the first consultation, so an application that excludes `/index.html` to serve that page through a handler of its own (a shell carrying a csrf token, or one behind authentication) would otherwise have it answered off the disk for the one spelling a browser actually uses. See [`Serve`](../../http/static/file_server.go).
* **The static file server declines a request path that is not already canonical.** `//css/app.css`, `/css//app.css`, `/css/app.css/` and `/./css/app.css` are not served; the request falls through the chain and typically ends as a `404` (the mount root answers the index file under its two canonical spellings, with and without the trailing slash, and under no folded one). The rule closes an access-control bypass — see [Canonical request paths](#canonical-request-paths) — and it has teeth, because **a browser does not normalise a doubled slash: it sends the path verbatim.** A template that concatenates a base path ending in `/` with an asset path beginning with `/` emits `/static//app.css`, that is what goes out on the wire, and the page renders with no stylesheet on a deployment where the same markup worked before. Fix the concatenation; that is the whole remedy in nearly every case. Where the url genuinely cannot be changed, name its prefix in [
  `MELODY_STATIC_EXCLUDED_PATHS`](#excluded-path-prefixes) so those requests reach the application and answer them with a handler of its own.
...
* The entity tag a static response carries is derived from the file's **size and modification time at nanosecond resolution** ([`GenerateEtag`](../../http/static/etag.go)), never from its contents — and from the size and the **build version** on a filesystem that reports no modification time, which is every embedded one. A rewrite that preserves both halves — a build step that restores timestamps to the nanosecond while keeping the length — carries the previous tag, so a revalidating cache is answered `304 Not Modified` and goes on serving the stale bytes; where a deployment can rewrite a file in place, put a content hash in the url instead of trusting the tag. The derivation is spelled out in the tag itself, so an anonymous client can read the modification instant — and, on embedded assets, the build version string — off any cached response; the major still under development publishes a digest of the same derivation instead. The tag, `Last-Modified` and `Cache-Control` are only emitted at all when static caching is enabled.
* **In the embedded mode there is no modification time at all**, and the validators say so. An `embed.FS` reports the zero instant for every file it holds, which made the size-and-time tag degenerate into size alone — identical across every rebuild — so the edge case above was the normal case, and a `Last-Modified` rendered as year 1 answered `304` to every `If-Modified-Since` for the life of the deployment, since the zero instant is never later than anything. The tag is therefore derived from the file's **size and the build version** ([`version.BuildVersion`](../../version/version.go)), and `Last-Modified` is not emitted. The consequence is coarser but honest: every embedded asset revalidates once after a deploy that changed the linked version, and no asset of the previous build survives one. A deployment that builds without `-ldflags` setting the version gets one version string for every build and, with it, the stale-asset behaviour back — link the version, or put a content hash in
  the url.
* **The public directory is proven against the embedded filesystem at construction.** `MELODY_PUBLIC_DIR` stays a runtime key while the embedded layout is frozen at compile time, so a value naming a directory the build did not embed used to boot cleanly and answer `404` for every asset in the binary; it now refuses by name at construction. The key is not ignored in this mode, because the join with the public directory is what confines a stripped prefix to it.
...
* **The kernel refuses a request path that carries an encoded separator.** `/admin%2Fusers` is answered `400` after the route is matched and before it is authorized or handled, and so is `/admin%2Fusers{`, whose escaped path net/url would re-spell from the decoded one: net/http decodes `%2F` into a `/` before the kernel reads the path, so the request would reach the `/admin/users` handler while a proxy or WAF rule written against the raw request line read one segment. A literal `%2F` inside a segment is sent as `%252F`; a value that carries a `/` belongs in the query string.
