...
A urlencoded body is buffered before `ParseForm` drains it, so a later reader — the HMAC body-hash check above all — still sees the raw bytes. When that read **fails**, the kernel refuses the request instead of dispatching it: `413` when the request-body ceiling stopped the read, `400` for a body the client broke mid-upload, negotiated between html and json like every other kernel refusal. The handler is never reached, because a failed read leaves a request whose form is indistinguishable from an empty submission — an oversized form would otherwise be processed as a blank one. A multipart body is left untouched (`ParseForm` does not read it), so the disk spooling for large uploads is preserved.
