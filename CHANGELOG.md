...
- tooling: `.dev/validate/apidiff.sh` files BOTH messages when apidiff reports two for one object, instead of the one that survived. apidiff keeps a single message per object and its `add` overwrites the earlier one, printing a `! second, different message` note to stdout and dropping the first from the report — so the band filed whichever message was added last. Measured on this tree that was `./debug.MiddlewareProvider` reported as a signature change, while the message apidiff destroyed said `removed` and the type has no declaration left in the module at all: a fabricated fact in the one file a release manager decides from, one row among the 128 incompatible ones still undecided. The note is parsed rather than forwarded, its two messages are joined onto that object's row and sorted — so the row reads the same whichever way a run resolves the two `add` calls, which nothing in apidiff orders — and a note whose object reaches no row at all now fails the run. The tool is installed at a PINNED pseudo-version of x/exp, the last commit before the module raised its go directive to 1.26 on 2026-08-20, for the reason the vulncheck band records: the container builds it with the image's own toolchain under `GOTOOLCHAIN=local`, so `@latest` was refused on a golang:1.25 image and the gate went red with the reason swallowed; the install's last lines now reach the failure message, and the pin moves together with the `FROM` line of the dev image

- tooling: the development PostgreSQL gets an init script of its own, so each example that keeps a set there holds it in a database of that major's own — `melody_example_v1` and `melody_example_v3`, beside the MySQL script that has done the same for the catalogue databases. It is the same reason, and it was not hypothetical: the bunorm migrate commands build their migrator on bun's default bookkeeping tables and bun matches an applied migration BY NAME, while the identifier of a migration is the leading digits of its file name — so the v1 example's journal, which lived in the shared `melody_test`, and a set written for another major on the same day would have carried the very same number, and the second one would have found its own step already applied and skipped it with no error, no line in `db:status` and no effect. The v1 example's journal moves off `melody_test`, which stays behind for the live integration suites that create tables of their own there. The script is one of two doors, as on MySQL: a volume provisioned before it exists never runs it, so `.dev/e2e/common.sh` creates the same databases idempotently — through a `pg_database` read piped to `\gexec`, PostgreSQL having no `CREATE DATABASE IF NOT EXISTS` and `psql -c` refusing a string that mixes SQL with a backslash command. The harness also gains `e2e_pgsql_scalar`, the PostgreSQL sibling of the MySQL out-of-band reader it has had all along: until now a PostgreSQL set could only be asserted through the application's own `db:<context>:status`, which is the application's word for the state rather than the state itself. `.dev/e2e/stack.sh` gains the `V3 READING ARCHIVE` section that reads through it

- tooling: the development stack serves an exchange-rate provider of its own, at `rates.melody.localhost.precision-soft.com`, and the v3 example calls it through `melody/v3/httpclient` — a package nothing in this repository imported on any major, so compilation was the whole of its proof. It is a vhost of the load balancer rather than a container or a public api: the gate has twice spent a session diagnosing external DNS and network degradation, and govulncheck is the one lane entitled to need the internet. The nominal path answers a constant document so every assertion about it is a value rather than whatever a third party quotes today, and the failure arms are BASES rather than targets — a provider that refuses, a body that is not a document, a status the reading cannot be taken from, and an address in TEST-NET-1 that accepts the connection and never answers — so pointing the application at one exercises a real failure through the real wiring instead of a second code path written for a test. The vhost names resolve to 127.0.0.1 through a public wildcard, which is what a browser on the host needs and what a container must not get, so the load balancer carries a compose network alias for the new name and the application reaches the provider the way a deployment would, by its name. `.dev/e2e/stack.sh` gains the `V3 EXCHANGE RATES` section and the e2e harness the conversion checks that spend the quotes
...
- tooling: `.dev/validate/vulncheck.sh` and `.dev/validate/vulncheck.baseline` run govulncheck over every Go module in the tree and gate on the REACHABLE findings, which nothing here had ever read — a vulnerable function this code actually calls was green in every other lane by construction. The baseline is bidirectional like the citation band's: a reachable finding with no row fails the check, and a row whose finding no longer occurs fails it just as loudly, which is what makes a toolchain bump visible instead of letting the rows rot into a suppression list. `all.sh` runs the lane under `--all`, gains a `--vulncheck` mode, and in `--staged` runs it exactly when a staged change moves a `go.mod` or `go.sum`; the ci mirror runs the same entry point. The tool is installed at a PINNED version, v1.7.0, where `@latest` let an upstream release decide whether the gate is green: the container builds it with the image's own toolchain under `GOTOOLCHAIN=local`, and x/vuln v1.8.0 started requiring go 1.26 on a golang:1.25 image, which turned every full run and the ci vulncheck job red on a release nobody here made, with the reason swallowed — the install's last lines now reach the failure message. The pin moves together with the `FROM` line of the dev image; the database the scan reads is fetched live whatever the tool version
- tooling: the pre-push hook verifies a proof of validation instead of running the gate inside the push. `git push` opens the connection and negotiates refs BEFORE the hook runs, so the full gate held the connection idle for its whole duration and the server's receive-pack timed out — git then died with SIGPIPE right after the hook reported success, a push that looked green and delivered nothing, and no transport keepalive can feed the remote process. A green `all.sh --all` now stamps the tree hash of the exact worktree content it validated into `.temp/validate-stamp`, and the hook recomputes and compares that hash in milliseconds: a match proves the pushed content is byte-identical to what the full gate saw, any later edit is refused by name, and the stamp's age is deliberately not judged because content identity is the guarantee. This also closes the hook's older gap of validating "the worktree" as a moving target — the stamp pins the exact tree the run proved. A push without a valid proof is still refused — that part is not negotiable — but the refusal STARTS the full gate in the background instead of only naming it, so the worst case is pushing twice: the first push kicks the run off, the second one passes on the stamp it wrote; a refusal while a gate is already running says wait instead of starting a second run against the same service containers, and answers "how far along" with a progress snapshot from the run's log — sections green so far and the section in flight — when the log is demonstrably the live run's. `./dc push` is the one-gesture form: with a valid stamp it pushes at once; without one it runs the full gate in the foreground, progress on the terminal, and pushes automatically on green; a gate already in flight is followed to its end instead of doubled. And the stamp itself is honest about time: the tree hash is taken at the START of the run and checked again before stamping, so an edit made while the gate ran — content no lane validated — finishes the run green and stamps nothing, with the reason named. And a push made only of commits the remote already holds — a new branch name on a pushed commit, `git push origin a:b` — passes without the stamp: the hook reads the ref updates git hands it on stdin and a sha reachable from a remote-tracking ref of the target remote is content that remote has, pushed once before through this same hook, so not one byte the gate has not certified leaves the machine; measured on a branch-off of a pushed HEAD in a fresh clone, refused for a stamp another clone held, that ran the whole gate for nothing. A sha the remote does not hold falls through to the stamp checks unchanged
...
- exception: **Behavioural change**: `Error.Unwrap` and `HttpException.Unwrap` answer nil on a nil receiver instead of dereferencing it. `errors.Is` and `errors.As` call `Unwrap` on every link of a chain, and a typed-nil link is the natural shape `FromError(nil)` produces when it is stored as another error's cause — `AsHttpException` over such a chain panicked on the request path, from inside the kernel's recovery, where the guards on the top of the chain could not reach it.
- exception: `FromError`, `FromErrorWithLevel` and `FromErrorWithLevelAndContext` — and `Logged`, which goes through them — render the message under the recover `LogContext` already uses, so an error whose own `Error()` panics is wrapped with the recovered text instead of raising a second panic inside the kernel's recovery defer that was reporting the first, which unwound past the 500 and the `kernel.exception` dispatch.
- internal: the deep-copy memo is keyed on the static type beside the pointer and the length. A defined type over `map[string]any` or `[]any` (a claims map, a `Meta`) shares its header with the plain value it was converted from, so the two spelled one memo key while being copied on different paths, and the typed fast path asserted the reflect path's copy — `Session.Get`/`Set`/`All`, the token store's claims and the route attributes copied on every match panicked intermittently, on whichever key the map iteration reached second (measured 35 runs out of 40 on a map holding both forms).
- internal: `RedactQueryValuesForDiagnostics` redacts each pair in place, in the order written, keeping every occurrence of a repeated name, instead of re-encoding through `url.Values`, which sorted the names and collapsed a repeated one: the internal-auth refusal, which fires only when the signed and the request query differ byte for byte and renders both side by side, showed two identical strings under an error saying they differ for a mismatch by reordering, by duplication or by multiplicity, and the access log rewrote the query line the client sent. A pair whose name does not decode still redacts the query whole; a pair with no `=` is kept as the bare name it is.
- bag: **Behavioural change**: `bag.String` on a repeated request key answers its FIRST value, as `Request.Input` and `url.Values.Get` do, instead of panicking toward `StringSlice`/`StringAt`. The panic reached every read through `bag.StringOrDefault` and `bag.HasNonEmptyString` — the documented way to read a request parameter — so one duplicated query key (`?name=a&name=b`) was a 500 an unauthenticated client could raise on any handler reading that key; the whole list is still read with `StringSlice`/`StringAt`, and an empty list reads as unset.
- application: an error handler installed after `Boot` returned no longer reads as installed while rendering nothing. The framework's exception listener answers every `kernel.exception` dispatch and the kernel consults the handler only when the dispatch produced no response, so a registered listener takes the handler's place entirely — and the decision was made at the end of Boot, which silently discarded a handler installed in the one window a composition root has left. An http process now makes that one decision where serving begins; a console process keeps making it at boot-end, so the dispatcher its introspection command reads still carries the same listener set a serving process runs
- application: an http shutdown whose serve or server-shutdown phase failed still drains the open request scopes. Returning on the first failing phase skipped the wait in the very situation it was written for — a hijacked connection is what the server's own `Shutdown` cannot drain and what makes it report a budget overrun — so the application container was closed with a handler still on the wire, and whichever causes the later phases would have reported were lost. Every phase now runs and each failing one contributes its cause; a lone cause travels exactly as it did before
- http: the session-cookie response guard no longer rewrites the `Cache-Control` it was only supposed to add `private` to. It read the header through `Get`, which answers with one field line, and wrote it back through `Set`, which replaces them all, so every directive on the other lines was deleted; and it split on a bare comma, which cuts through a quoted field-name list, so a fragment left holding a field name spelled like a directive was dropped as if it were one — `no-cache="X-One, Public, X-Two"` came back as `no-cache="X-One, X-Two"`. Every field line is now read, and the split honours the quotes
- logging: a step of the exit handler that PANICKED is no longer reported as a step that finished. The shield contains the panic on the step's own goroutine and closed the done channel from a defer, which fires on both paths, so the one caller that reads the answer — the clean shutdown, which ends the process when a teardown could not complete — was told the teardown had run to its end, and a process that lost everything after the failing step exited 0. The channel now carries the outcome rather than the fact that the goroutine ended
- security: a listener marked may-skip that FAILS no longer suppresses the required-listener refusal. The mark licenses a listener that knowingly short-circuits, which is what stopping propagation is, and both its own documentation and the registrar contract scope it there; read on the failure branch it granted more than the stop it was written for, so a listener that failed after setting a response had that response served with the required access-control listener never consulted — the kernel tells the two cases apart by the TYPE of the error the dispatch returns, and it never saw one
- logging: the json logger no longer holds its write lock while it echoes a failed write to stderr. The echo exists to report that the journal has stopped writing, and it writes to a channel that can be a pipe to a collector that stopped reading: parked there under the lock it parked every goroutine that logs, and `Close` with them, on the one report that says the journal is gone
- logging: the json logger no longer runs the caller's own code under its write lock. A context value renders through a `MarshalJSON` or a `String` of the application's own, which is unbounded work this logger does not own and which may itself log — under the lock, such a value deadlocked the journal on its own record, and every other writer queued behind the slowest caller's encoder. The normalization and the encoding of the context both happen above the lock now; what runs under it is assembly of already-encoded values and the write
- logging: the json record's timestamp is written to a fixed width in UTC, so the stamps sort as text into the order they were written. The stamp is taken under the write mutex precisely to make the stamp order the write order, but `RFC3339Nano` trims trailing zeros — a record landing on a whole second rendered without a fraction and sorted after every fractional record of that same second — so the ordering the lock was paying for was not in fact delivered
- logging: an error assembled by `errors.Join` reaches the record with every branch it carries. The record's context was walked from `errors.Unwrap`, which answers nothing at all for a joined error, so a failure that had gathered what several replicas, several destinations or several rules had to say arrived with no cause, no chain, and the context of whichever branch happened to be found first
- logging: an error whose own `Error()` panics still produces its record. The message was taken straight off the error, on a path reached from inside the recovery handlers, where the error is whatever a panic carried — an `Error()` dereferencing the very nil field that made it panic-worthy took down the one record written to explain the failure
- logging: the final stderr echo before a fatal exit is bounded, so a stderr nobody drains cannot hold the process short of its exit. It was the one step between the resolved exit code and `os.Exit` that ran unbounded, and a clogged pipe parked the process on the line that says it is dying. It is bounded rather than shielded because the shield reports an abandoned step on the very channel that is blocked
- security: a login or logout whose failure event could not be dispatched names the listener that broke. The dispatch error was flattened into a context slot, where it rendered as the bare "event listener returned error" — the listener's name, the event and the listener's own cause all live in that error's context, and a string slot drops them at the render boundary. Both failures travel as causes now, the handler's first
- event: the already-logged mark of a panic value is read at the depth it is written. A shallow type test answered only for a payload carrying the mark itself, so a listener that panicked with an error wrapping one whose producer had already logged it got a second record for the one failure — where the same file reads the mark correctly of a returned error, and the logging package's twin delegates correctly of a panic payload
- event: the zero value of `RequiredListenerSkippedError` answers instead of dereferencing. The type is exported with an unexported error field, so the zero value is constructible outside the constructors that always set it, and the http kernel reaches this type by assertion and then renders it; `Error()` and `Unwrap()` now answer the way the sibling this type is shaped after does, with the unwrap link a real nil rather than a boxed typed nil
- http: a query the client wrote with a legacy semicolon separator no longer refuses the form beside it. `ParseForm` parses the body and the query and reports the body's failure first, falling back to the query's, so the one return value could not say which half broke: a POST whose urlencoded body was perfectly valid was answered `400`, and the handler's form was emptied on the way there. Only a body that fails to parse is refused now, and a body that parsed reaches the handler whatever the query did
- http: a cookie the handler writes on the response writer its own contract hands it survives beside the framework's session cookie. The response owns a header it names, and the writer's values for that key are replaced — but `Set-Cookie` lines are separate cookies, not one field restated, so "both sides set it" meant two DIFFERENT cookies and the handler's was deleted in silence. Every other header the response names is still owned by it
- http: a `q` parameter written with excess trailing zeros keeps its member in the negotiation. The qvalue grammar carries three decimals, and a fourth digit was refused on count alone — so `application/json;q=1.0000`, which says exactly what `q=1.0` says, dropped its member entirely and an api client asking for json was served the html error page. Zeros past the third are read; a non-zero digit there still carries a weight the grammar cannot express and its member is still dropped, and `q=0.0000` still refuses
- http: the sliding window limiter no longer hands a caller its budget back when the wall clock moves backwards under the process. The marks are searched with a binary search, which is entitled to an ordered slice; an instant recorded out of order made the predicate non-monotone, and the search could cut away a mark still inside the window rather than merely keep an expired one — a full replenishment on a limiter aimed at login, one-time codes and password reset. The recorded instant is clamped to the last mark, which can only shorten the caller's own budget
- http: a panic raised by the response body reader while the response is being compressed no longer takes the process down. The copy runs on a goroutine the compression middleware starts, where neither of the kernel's recovery defers nor net/http's own per-connection recovery stands, so one handler's bad body killed every in-flight request with it. It is contained and reported as the copy error the reader of that response already fails on
- http: a duplicate route an aggregating boot records instead of refusing no longer enters the matching tree under another route's index. The index the tree receives is the position of the last STORED route, and a declined registration stored nothing — so the pattern's tree entry named whichever route was registered before it, breaking the invariant every reader of the tree relies on and the one the priority tie-break reads the index for
- http: the response discarded for a status code outside net/http's accepted range is closed before it is replaced. A file response left its `*os.File` open for the life of the process, one descriptor per request that reached there, where the sibling branch that discards a response on a session-storage failure already closed it
- http: a flush issued by a streaming handler reaches the connection through the whole wrapper chain. The kernel's recording writer forwarded the flush to its immediate delegate, so a wrapper between it and the connection that implements `Unwrap` without a `Flush` of its own — the shape an operator's own net/http middleware takes for ResponseController compatibility — turned every flush into a silent no-op: the frames sat in the buffer, the handler saw no error, and the client received nothing until the response ended
- session: `Session.Set` stores a copy of the value it is handed, at the depth `Get`, `All` and `Snapshot` already copy at. The write door kept the caller's map by reference while every read door deep-copied, so the session held memory it did not own: a caller that went on writing to the map it handed over raced the copy the response path makes, and a concurrent map read and write is a fatal Go error that takes the process down with no recover reaching it. The leak ran the other way too, a later write by the caller silently becoming part of the stored session
- session: a `WriteAt` that fails PART WAY through the session file is reported as a write that already disturbed the document, not as one that changed nothing. The byte count was discarded, so a torn write — the head of the new document over the tail of the old — rolled the in-memory entry back to a state the file no longer held, the same inversion the truncate and sync failures beside it are already marked for
- documentation: the test comments carry no reference to melody's internal lesson register. Five `(§5.N)` citations had been written into test files across all three majors; the numbering belongs to a maintainer document that is not published, so a reader of the repository had nothing to resolve them against
- logging: a log context that closes on itself is rendered instead of killing the process. The context normalization bounded its descent and then handed whatever sat at the floor to the encoder as it was, so a self-referencing map or slice reached `json.Marshal`, which answers a cycle with an ERROR — and that error routed the record into the `fmt` fallback, which has no cycle detection of its own and recursed until the goroutine stack was gone. A stack overflow is a fatal error that no recover turns into a reported failure, so the record written to report a failure took the process down with the write mutex held. The walk now keys the containers on the path it is currently descending and renders a loop as `<cycle>`, a defined type whose underlying type is one of those containers included; a subtree past the bound renders as `<depth limit>` rather than travelling on unwalked; and the bound itself became a stack backstop rather than a shape limit, so an error nested deeper than a handful of levels renders as its message instead of as the empty object
- session: `Manager.DeleteSession` buries the tombstone only once the storage removal has actually succeeded. It buried first and removed after, and nothing lifts a burial before its retention window lapses, so a transient storage outage on a logout or a session rotation retired an id whose entry was still there: the caller's next `SaveSession` answered `ErrSessionDeleted`, and the response path read that refusal as a deliberate logout and expired the browser cookie — a storage blip logged the user out of a session that had never been removed, for the whole retention window. `RegenerateSession` already skipped its `Clear` latch on a failed delete and already promised the caller a session it can keep using; that promise now holds. The burial stays inside the same per-session critical section, so a save waiting on that lock still cannot slip between the removal and the tombstone
- logging: the control-character escaping covers the C1 block (`U+0080`–`U+009F`) and the Unicode line and paragraph separators (`U+2028`/`U+2029`), not C0 and DEL alone. A terminal decoding UTF-8 obeys a C1 rune the way it obeys the two-byte ESC sequence it abbreviates, so a single `U+009B` repainted a line no ESC ever entered; and `U+0085`, `U+2028` and `U+2029` are record boundaries for every reader that splits on Unicode line boundaries rather than on `\n` alone, so one escaped record was read downstream as two. Both reach the escaping through attacker-controlled text: a request path is percent-decoded before it enters the log context, so `%C2%9B` arrives as a real rune, and this helper is the only sanitiser standing between it and the record. A rune above one byte is spelled `\uNNNN`, because `\x2028` reads as `\x20` followed by two digits; C0 and DEL keep `\xNN`, and the cell form still keeps its real line breaks
- httpclient: the diagnostic url sanitizer cuts the userinfo wherever the reference carries one. A scheme-relative authority — `//user:secret@host:notaport/path` — has no `://` in it, so a parse failure on the port, on a control character, on a broken percent escape or on an unclosed bracket sent it to the textual fallback, which returned it with the credential intact; and an opaque url — `http:user:secret@host/path`, one slash missing — parses successfully with a nil `User`, so the parsed branch handed the credential back without ever reaching the fallback. Both land in the error contexts built expressly to strip credentials, and the sibling that reports the parse failure drops the url whole precisely so this one holds it. The authority is now located after a scheme separator, after a leading `//`, or after a scheme's colon, with the scheme read by its RFC 3986 grammar so a relative path carrying a colon is not mistaken for one
- cli: the run banner honours `quiet`, the documented governor of decoration: under `StandardFlags`, whose quiet defaults to true, a scripted invocation reads the command's own output alone, and one explicit `--quiet=false` brings the frame back; `DebugFlags` commands keep their banner, since their quiet defaults to false. The banner ignored the flag entirely, so the one output the contract promises quiet suppresses was the one it never touched
- logging: an error sitting exactly at the context normalization's depth floor renders as its message instead of reaching the encoder raw, which marshalled it as the empty object — the floor bounds the descent, not the scalar conversion, and the record lost precisely the failure it nested that deep to carry
- debug: the resolution-failure cause chains of `debug:container --build` and `debug:middleware` are walked from the failure itself with the head dropped, instead of from a bare `errors.Unwrap` — a joined failure answers that with nothing, its causes living behind the `[]error` shape, so the report lost every cause exactly when there was more than one to show
...
- session: the file storage keeps the in-memory state when a flush step fails AFTER the new document is already on disk, through either writer. The handle-owning path lays the whole snapshot at offset 0 and a single decode reads exactly it; the path-owning one renames the temp file over the target before it fsyncs the directory the name lives in. Past those points the file holds the new data, so rolling the in-memory entry back made it disagree with the disk — a "failed" save whose data was persisted, or a "failed" delete that had already removed the session, with memory serving the old values until a restart read the file and resurrected the write the caller was told had failed. The rollback is skipped for every failure the commit already outran: `Truncate`, `Sync`, and the post-rename directory fsync
...
- security: `PathPrefixMatcher.Matches`, `ApiKeyHeaderRule.Check`, `FirewallRegistry.Match` and both kernel listeners read the request through the interface instead of comparing it to nil. A nil pointer of a request type is a non-nil interface a bare comparison carries through, and the dereference that follows took the request down with a nil pointer panic on the path that decides which firewall claims it — the guard already standing there for a nil request could never see the shape an application actually hands over.
- http: the same reading now covers the request everywhere the framework holds one — `PrefersHtml`, `RegenerateRequestSession`, `Service.IsPreflight`, `Service.RequestOrigin`, `FileServer.Serve` and `FileServer.ServeReader` among the public doors, with the exception, profiling, terminate, cors and rate-limit listeners and the router's session readers behind them. Every one of those sites answered a typed nil with a panic rather than the refusal it was written to give.
- session: the session carries one cleared flag where it carried two. `Clear` raised both and `Set` lowered only one, so the second already decided every read; the write `Set` made had no reader left and the flag it wrote could not be observed.
- http: the error renderer tests the serializer resolution once rather than three times. The not-acceptable term could never be the one that decided — the error test beside it covers it strictly — and naming it there claimed a branch this door deliberately does not have: an error keeps its own status rather than being masked behind a 406, which is where the success path and this one part company.
- session: the in-place write probe answers a distinct token for each way it can exit, and runs in a directory of its own. It wrote the success token on three paths where nothing had been injected, and its fixed name under the shared temp directory had the children of two majors seed and remove the same file.
- tooling: the dev image is rebuilt onto the go1.25.13 base and `.dev/validate/vulncheck.baseline` is empty again. The eighty-three rows it carried were one class — advisories against the standard library of the go1.25.12 toolchain the image had cached, each already fixed in go1.25.13 — and the band is bidirectional, so every one of them failed the check as a stale row the moment the image moved. Nothing in the tree pins the standard library, so no `go.mod` could have retired them; the rebuild is what does. The run now scans thirty-three modules with zero reachable findings and zero baseline rows, which is the state the band was built to make visible rather than let rot into a suppression list
- documentation: the `Unreleased` section of `.documentation/UPGRADE.md` names the release its sections shipped in. The sentence introducing them said they were the changes "currently sitting in the `[Unreleased]` block" of this changelog, which stopped being true when `v1.19.0` went out on 2026-08-18: that block now holds a different set of patch-level entries, none of which asks the upgrader for an action, so a reader following the pointer landed on the wrong list. The heading itself stays `Unreleased`, because this guide promotes at a MAJOR boundary — `v3/.documentation/UPGRADE.md` carries `v3.0.0` and no minor heading despite thirteen minor releases — and the sentence now says so
- container: a lazy handle serves the service closing through it for as long as the container still answers that question directly. The container raises the flag `IsClosed` reports at the START of its teardown and stops answering resolutions only once the last service `Close` has returned — the gap between the two is deliberate, and it is what entitles a service's own `Close` to the things it depends on. `LazyService` read the earlier flag, so for the whole of that window a handle answered the scope-is-closed error and dropped its memoized value on the way, one-way: a worker releasing a lease or draining a queue from `Close` was refused by the handle and served by the resolver beside it, and the handle stayed poisoned afterwards. It now asks whether a resolution would be refused, which is the question the resolver itself answers, both for a handle built over the container and for one built over the resolver a provider was handed — the shape the `LazyService` documentation recommends when the teardown ordering matters. A handle over a request scope is unchanged and still turns terminal with its request, because a scope stops answering the moment it is marked closed, so its two doors already agreed
- application: a module registered as a struct VALUE carrying an `any` field boots instead of panicking. `RegisterModule` skips its identity map for a module that cannot be a map key, and it asked the question of the module's TYPE — where an interface field counts as comparable whatever it ends up holding. A module whose field held a map, a slice or a func therefore passed the guard and panicked with `hash of unhashable type` on the very next line, which is the exact panic the guard exists to avoid. The question is now asked of the VALUE, the way the container already asks it in `isComparableValue`; a module whose field holds something hashable keeps its identity, so one instance reached through two providers still boots once
- container: a handle captured by a service the CONTAINER owns outlives the request that happened to build it. Such a service is one instance for the whole process, but its provider first runs inside whichever request reached it, and the resolution view it is handed still carries that request's scope — suspended, so the provider's own wiring cannot read it. The liveness question consulted only whether a scope pointer was present, so the view answered for that one request: a `LazyService` the singleton captured turned terminal and dropped its value the moment that request ended, while the container went on answering the same name directly for the rest of the process, and the next request was served a singleton whose handle was already dead. The question now follows `scopeVisible`, the same predicate that decides where the resolution reads, so the two agree: a suspended view reads the container and ends with it
- logging, cli: the control-character escaping renders a byte that is not part of a valid UTF-8 sequence as `\xNN` of that byte, whatever its value. The escaping walked the value rune by rune, and a raw byte decodes as U+FFFD, outside every range it checks, so the single-byte C1 introducer `0x9b` — which a header value admits and a percent-encoded path segment is decoded to — passed through as sent, contradicting the promise of `\x9b`; and once any other control character forced the rewrite, that byte was replaced by U+FFFD in silence, so the same input was either kept or corrupted by what stood beside it. The output is now always valid UTF-8, and a genuine U+FFFD passes through as itself
- cli, logging: the json document of `--format=json` and every record of the json logger spell the C1 block (U+0080 to U+009F) as `\u00NN` escapes. `encoding/json` escapes the C0 block, the two Unicode line separators and the html-significant runes, and emits the C1 block raw, so a document or a record carrying U+009B repainted the terminal it was printed or tailed on and a NEL ended a record for any reader splitting on Unicode line boundaries; the escape decodes to the same rune, so a consumer reads the value it was given. A byte that is not valid UTF-8 reaches the document as U+FFFD, the encoder's documented answer, which the rewrite does not undo
- cli: the finish banner colours the `[failed]` verdict after the escaping rather than before it. The coloured verdict was built from the banner's own escape sequences and then handed to the escaping that keeps client-derived text from repainting the line, so every failed run with colour on — the default — printed `\x1b[31m` as literal text around `[failed]`, while `--no-color`, which never coloured the verdict, printed it right. The text on either side of the verdict is escaped as data and the colour is added last

### Security

- security: a firewall whose matcher is a typed nil is skipped instead of crashing the walk that decides which firewall claims the request. `NewCompiledFirewall` is public and validates nothing, so a nil pointer of an application's own matcher type arrives as a non-nil interface that `nil ==` reads as live, and the `Matches` call dereferenced it, on EVERY request. The login and logout handlers and the entry point and access denied handler beside it are read the same way now
...
- http, security: the kernel refuses, with a 400, a request path that leading or trailing whitespace would be trimmed from — the decoded `/public%20`, `/public%09`, `/public%C2%A0` — before it is routed, authorized or handled, the way it already refuses a path that folds. The router keeps the whitespace, so such a request reached the catch-all handler under `/public `, while the access-control matcher trims it and authorized the request under the rule of `/public`: an exact `PUBLIC_ACCESS` rule beside a protected catch-all handler served the protected handler to an anonymous client — measured through a real kernel and router, over a socket, on the releases that carried the trim and the untrimmed router together. The matcher keeps its trim, because a matcher without it leaves the whitespace spelling with no rule, which grants as well; the sentence in its documentation that read the trim as a refusal and never a grant is rewritten to what was measured. Whitespace inside the path is read alike by every consumer and is not refused. A path that BEGINS with whitespace is refused the same way: Go's own server refuses such a request line, but a handler mounted in front of the kernel that rewrites the path — the standard library's `StripPrefix` on `/api%20/public` — handed the kernel ` /public`, which the guard let through as a target that is not path-routed, the router routed as a segment of its own and the matcher trimmed to `/public`: the leading twin of the same grant, measured on the releases as shipped
