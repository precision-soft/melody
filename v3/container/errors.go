...
/* ErrTeardownDependencyWasNeverRegistered is the cause of an arming refusal — and, once armed, of a Register refusal — for a declared teardown dependency naming a service nothing ever registered, under any spelling. The sequential teardown drops such an edge in silence, because the creation order carries the ordering anyway; under waves the dropped edge is the ordering itself, so a misspelling is refused where the author can still fix it. A service that was registered and never built is not this: that is the optional collaborator the declaration exists for. */
