...
func TestRouterCommand_TiedRowsFollowTheRegistrationOrderAndNotTheName(t *testing.T) {
    ...
    for index := 0; index < 8; index++ {
        router.HandleWithOptions(
            ...
            http.NewRouteOptions(
                fmt.Sprintf("tied.reversed.%02d", 7-index),
                []string{nethttp.MethodGet},
                fmt.Sprintf("host%02d.example.com", index),
                nil,
                nil,
    ...
    if decodeErr := json.Unmarshal([]byte(rendered), &named); nil != decodeErr {
        t.Fatalf("failed to decode the rendered envelope: %v, rendered %q", decodeErr, rendered)
    }

    if 8 != len(named.Data.Items) {
        t.Fatalf("expected 8 rows, got %d", len(named.Data.Items))
    }

    for index, item := range named.Data.Items {
        expectedName := fmt.Sprintf("tied.reversed.%02d", 7-index)

        if index+1 != item.Order {
            t.Fatalf("expected the tied rows in registration order, row %d carries order %d", index, item.Order)
        }

        if expectedName != item.Name {
            t.Fatalf("expected row %d to be %q, got %q — the listing followed the name, not the registration order", index, expectedName, item.Name)
        }
    }
}
