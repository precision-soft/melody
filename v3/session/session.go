...
/* Clear permanently marks this session ended; later Set calls cannot revive it. Ask the manager for a new session when needed. Clear must complete before the handler returns: a late goroutine can miss the response path’s session snapshot. */
