...
/* with a memoized result and a lapsed caller context both ready, a select chooses at random, so the tie is built sixty-four times and every one must answer the value */
