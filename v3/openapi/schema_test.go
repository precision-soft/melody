...
/* the semantic lockstep with the validator's parseIntStrict is proved in lockstep_test.go through the validator itself (TestLockstepNumericBoundAgreesWithValidator); this pins only the strict acceptance in isolation — an integer in its entirety, where the pre-repair parse tolerated trailing junk, read a float's leading digits and accepted surrounding space. */
