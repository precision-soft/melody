...
/* RFC 2047 §5 forbids encoded-words inside the msg-id tokens of these headers, so they are emitted intact, still CRLF-stripped and folded by writeHeader, and never Q-encoded */
...
/* writeRelatedEntity writes a multipart/related entity (RFC 2387) wrapping the body and then the inline attachments; its type parameter is the media type of the root part writeBodyEntity emits. */
...
/* validateStructuredIdentifierHeader refuses a Message-ID, In-Reply-To, References or Content-ID header that the msg-id grammar cannot carry intact: any control or whitespace rune but the single space between tokens, which writeHeader would not strip, and a token too long for a continuation line, which folding would split mid-token. */
...
/* validateInlineContentId refuses a Content-ID, a single msg-id token, that carries whitespace or a control character, or that is too long for one 998-octet continuation line with its leading space, since folding would corrupt it. */
func validateInlineContentId(contentId string) error {
    ...
    /* angle brackets are valid only as one matched leading and trailing pair, so a value such as <>x is refused rather than bracketed into a malformed Content-ID */
