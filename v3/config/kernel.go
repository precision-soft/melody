...
/* DefaultTeardownTimeout is how long a process shutting down cleanly may spend releasing what it holds before the teardown is abandoned and the process exits non-zero, when MELODY_TEARDOWN_TIMEOUT says nothing. It is the budget the whole ordered teardown spends, not one per service, because the services close serially and a supervisor budgets for the process rather than for its parts.
