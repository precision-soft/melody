...
/* the four slice accessors hand out a copy. The service is a process-wide singleton read from every request goroutine, so a caller that mutated what it received would rewrite the policy of every request that follows — and OriginAllowed reads the very slice AllowOrigins returns. */
