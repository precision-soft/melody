...
/* Open resolves a name under the base directory and refuses what would leave it, the way confineFileToRoot in the http package does for a handler's file response, with the differences that door's own comment names: a directory is admitted here because the caller dispatches directories itself, a base that cannot be evaluated falls back to its raw spelling, and the refusals are the fs errors this io/fs contract prescribes. A change to what "outside the root" means is made in both.
