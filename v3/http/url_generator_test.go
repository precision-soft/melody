...
func TestGeneratePath_EscapesSlashInParam(t *testing.T) {
    ...
    pathValue, err := urlGenerator.GeneratePath("file", map[string]string{"name": "a/b"})
    if nil != err {
        t.Fatalf("unexpected error: %v", err)
    }

    if "/file/a%2Fb" != pathValue {
        t.Fatalf("expected slash to be escaped in param, got: %s", pathValue)
    }
}
