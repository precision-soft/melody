...
/* Migrations is the single source of this example's schema on mysql. The five repository constructors and the provider of the two-factor store run it at first use through EnsureMigrated, and the db:* command family the bunorm/migrate module registers runs the same set from the operator's side, so neither door can drift from the other.

   The set is a SINGLE migration holding the whole schema, because this application has no history: an example has one state, the present one, and the schema is the statement of that state rather than the record of how it got there. A volume left in an older shape is brought to the present one by example:db:reset, not by a step that repairs its past — and it is REFUSED by name until it is: the set is recorded as applied by name and its tables are created IF NOT EXISTS, so it passes such a volume untouched, and the first resolution compares every table the set creates with the one the volume holds (see refuseSchemaDrift) instead of letting the first request that reads a missing column answer 500.

   Every statement of that migration tolerates a second run of the set on the same volume — the run it began and did not finish, which beginSchemaSet lets it finish while it refuses a volume it did not build — and several processes of this example applying it at the same time: the tables are created IF NOT EXISTS, and the constraint — MySQL having no ADD KEY IF NOT EXISTS — is added only after the catalogue is asked whether the key is already there. The bun bookkeeping tables keep their default names (bun_migrations and bun_migration_locks), because the module builds its migrator without the table-name options; they live in this example's own database, which no other major writes, so nothing else touches their rows. The identifier is the same on all three majors, which is harmless for the reason it was ever allowed to differ: each major holds its schema in a database of its own, so two sets never meet in one bun_migrations table.
