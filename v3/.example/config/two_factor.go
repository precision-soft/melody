...
/* the two-factor demo persists a user's TOTP secret and single-use recovery codes (encrypted at rest via
...
func (instance *Module) buildTwoFactor() {
    ...
    if schemaErr := store.EnsureSchema(context.Background()); nil != schemaErr {
        /* a demo table-creation failure (for example the database is briefly unavailable at boot) leaves
