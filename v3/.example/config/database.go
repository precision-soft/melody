...
const (
    ...
    /* databaseManagerName names the one manager this example declares. The db:* commands take it as their default because the definition is the registry's default, so omitting --manager can only ever reach this connection. */
...
/* buildDatabase declares the connection in a bunorm manager registry and opens the registry's default manager. The registry validates the definitions as it is built, and the handle the rest of the application holds is that manager's — the operator running db:migrate and the repository resolving at first request reach one pool rather than two.

   An unset host leaves both the registry and the handle nil, and the whole database surface unwired: no services, no migration set, no dial. */
func (instance *Module) buildDatabase() {
    ...
    /* the emergency logger carries the registry's reporting: the framework's own logger does not exist yet while the modules are being wired, and the emergency one is what the framework itself writes through in that window. The password is not marked here the way the two frozen majors mark it through the registry: this major registers its parameters itself and marks MYSQL_PASSWORD by name in RegisterParameters, so a second marking would say the same thing twice. */
