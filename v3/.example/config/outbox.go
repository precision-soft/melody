...
/* serviceOutboxTransport is the container name of the outbox's own amqp transport. It is a registered
service rather than a private object of the relay factory for exactly the reason the outbox module's
GoDoc warns about: a connection opened inside a factory is invisible to container teardown, so the amqp
connection lived exactly as long as the process. Registered, the transport's Close() error joins the
ordered teardown once the relay has resolved it. */
...
/* outboxRelayFactory is the service.outbox.relay provider: it resolves the registered store and the
registered transport when the relay is first used — the built-in melody:outbox:relay command resolves its
relay the same way, so an http-mode process registers the command without paying for the amqp connection. */
...
/* buildOutboxTransport hands the transport ONLY a dialer, no pre-opened connection: the transport closes
a connection it dialed itself, while one opened here and handed over would be owned by nobody — the exact
leak registering the transport exists to close. The first publish dials; a bad DSN surfaces there through
the relay's own backoff-and-retry loop rather than killing the resolution. */
