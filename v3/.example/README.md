...
- A reading carries its instant in two reference frames. `asOf` is stamped on the PROVIDER's clock, and the
  ...
  replay keeps an old stamp under an answer given now, so it is older here. The offset is trusted within five
  ...
  readable date or with an `Age` above the largest a cache may send — its stamps are then taken as they came, no
  later than the moment the answer arrived, and one more than five minutes ahead of this clock is refused.
...
refused where the wiring mistake is made. The shipped value is plain `http://` only because it names the
