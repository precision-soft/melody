...
func (instance *UserService) GrantRole(runtimeInstance melodyruntimecontract.Runtime, username string, role string) (*entity.User, bool, error) {
    ...
    switch role {
    case entity.RoleUser, entity.RoleEditor, entity.RoleAdmin:
    default:
        return nil, false, fmt.Errorf("unknown application role %q", role)
    }
    granter, supported := instance.userRepository.(interface {
        GrantRole(context.Context, string, string) (*entity.User, bool, error)
    })
    if false == supported { return nil, false, fmt.Errorf("the user repository does not support atomic role grants") }
    account, changed, err := granter.GrantRole(WriteContext(runtimeInstance), username, role)
    if nil != err || false == changed { return account, changed, err }
    _, err = instance.eventDispatcher.DispatchName(runtimeInstance, event.UserUpdatedEventName, event.NewUserUpdatedEvent(account, account.Username))
