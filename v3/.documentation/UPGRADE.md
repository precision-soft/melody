...
The access-control matcher, the firewall path matcher, the kernel's canonical-path guard, the static file server and its exclusion list, the compression exclusion list and the internal-auth endpoint binding read that same spelling (`http.RequestPathAsRouted`), not the decoded `URL.Path`: `/public%2F` is the resource `public/`, distinct from `/public`, so an exact rule written for `/public` does not claim it — measured, read decoded it was folded onto the public rule of `/public` while the router served it through a protected catch-all handler, to an anonymous client — and a firewall or rule written for `/admin/` does not claim `/admin%2Fusers`, a one-segment resource the router never routes under `/admin`. A rule that means to claim a resource whose name carries a separator names it with `%2F`, and so does an internal-auth caller signing the path of such a request (`HmacEnvelopeSigner.Sign` is handed the path in the routed spelling — each segment decoded, a separator encoded inside a segment kept as `%2F`: `/files/a%2Fb` for that resource, `/files/café` for one whose segment carries no separator, never the escaped request line). The static file server resolves a file from the routed spelling, so `/static/private%2Fsecret.txt` names a file whose name carries `%2F` and no longer reaches `private/secret.txt` beneath the mount.
...
### Bunorm mysql and pgsql: a zero pool or timeout field takes the default, and `Unlimited` lifts a bound

**What changed.** Every zero field of `PoolConfig` and `TimeoutConfig` in the [`mysql`](../../integrations/bunorm/mysql/v3/provider.go) and [`pgsql`](../../integrations/bunorm/pgsql/v3/provider.go) providers falls back to the constructor default, and every negative one lifts the bound, which the new [`Unlimited`](../../integrations/bunorm/mysql/v3/unlimited.go) names: no deadline for a timeout, no cap on the open or idle connections, no recycling for a lifetime or an idle time. A zero used to reach `database/sql` and the driver as it was, where it means an unlimited pool, connections never recycled and, on the mysql driver, no read or write deadline; and a negative connect timeout failed every mysql dial at once.

**Symptom.** A configuration that set a field to zero to mean "no limit", as the released changelogs of mysql v3.1.0 and pgsql v3.1.1 described for `ConnectTimeout`, now runs under the default: a connect timeout of 10 seconds on mysql and 5 on pgsql, a pool of the default size, connections recycled on the default lifetime. The same happens to a zero that came from an environment key nobody set, which is the case the change is for.

**Remedy.** Set the field to `Unlimited` (or any negative value, such as `-1s` read from configuration) where no bound is wanted, and leave the rest at zero for the defaults. The migration connection keeps its lifted read and write deadlines and its disabled recycling whatever the configuration says.
...
**What changed.** Every file the builtin `k8s` template renders starts with three comment lines carrying the ownership line — `# owned by melody:cron:generate for <cli name>`, the application's name after the command since the section above —, the same line the crontab dialects carry in their header block, and the template renders the marker header alone — demanding no container image — when it has no entries. That is what lets `--prune` reconcile a k8s output directory: the sweep empties only a file whose leading line, matched whole, proves this application's generator wrote it.
...
**What changed.** A relative path read from a parameter — `melody.cron.destination_file`, `melody.cron.logs_dir`, `melody.cron.heartbeat_path`, `melody.cron.binary` — is anchored at the project directory, the way melody resolves `kernel.logs_dir` and its siblings. A relative path typed as a cli flag stays relative to the working directory, as a shell path should.
...
* **`bunorm` async audit storage.** [`audit.AsyncStorage.Save`](../../integrations/bunorm/v3/audit/async_storage.go) returns an error wrapping `ErrAsyncStorageQueueFull` or `ErrAsyncStorageClosed` for an entry its queue could not take, where it answered `nil`; the entry is still dead-lettered, and the error names every refused entry under `refused` with its position in the call. *Symptom:* a request path that propagates the audit write's error fails when the queue is full or the storage is closing. *Remedy:* match the two sentinels with `errors.Is` and decide per call whether a lost audit record fails the request or is only journalled.
* **`bunorm` audit change presence.** A [`audit.Change`](../../integrations/bunorm/v3/audit/change.go) the tracker builds marshals each side by presence: a transition to a zero value renders it (`"new":false`, `"new":0`, `"new":""`), where `omitempty` left that side out and made it read like an insert or a delete. A `Change` built by hand keeps the old reading. *Symptom:* a consumer of the `changes` column that took an absent side for "no new value" now sees the zero. *Remedy:* read a present key as the value it carries and an absent key as no value recorded.
* **`bunorm` audit table name.** [`audit.BunStorage.Save`](../../integrations/bunorm/v3/audit/storage.go) refuses, as an error, a table name that is not a plain identifier, the empty name included, since the name reaches the insert as raw SQL; it used to substitute the default table for `""`. *Symptom:* a direct caller that passed `""` gets `audit table name is not a valid identifier` instead of a write to the default table. *Remedy:* pass `audit.DefaultTable`, or the name the registry hands out.
* **`bunorm` audited delete.** [`Tracker.Delete`](../../integrations/bunorm/v3/audit/track.go) records the identifier, the actor and the time, and no field values, unless the entity opts in with [`EntityOptions.CaptureDeleteBeforeImage`](../../integrations/bunorm/v3/audit/config.go), which loads and locks the row before removing it; a delete that matched no row is not recorded. *Symptom:* a delete entry carries no old values where it carried the fields of the model handed in, and a delete of an absent row leaves no entry. *Remedy:* set `CaptureDeleteBeforeImage: true` on every entity whose deleted contents must be recoverable from the trail; with it, deleting a row that is already gone is an error.
* **`bunorm` pgsql duplicate key.** [`pgsql.IsDuplicateKey`](../../integrations/bunorm/pgsql/v3/pgsql_error.go) answers on the typed SQLSTATE `23505` a PostgreSQL protocol error carries, read through the wrapping, instead of searching the error's text. *Symptom:* an error that carries no protocol error — a hand-written message containing "duplicate key", an error stringified across a process boundary — no longer matches. *Remedy:* keep the driver's error in the chain (`%w`, or an exception's cause) where the check has to see it.
* **`cron` busybox day fields.** The user-less crontab dialect ([`template_crontab.go`](../../integrations/cron/v3/template_crontab.go)) refuses, with `ErrBusyboxDivergentDaySchedule`, a day-of-month and day-of-week pair that busybox crond runs as a different schedule than vixie crond and the in-process runner. *Symptom:* `melody:cron:generate` fails under that dialect for an entry it used to render. *Remedy:* restate the pair so both daemons read it the same, usually by leaving one of the two day fields at `*`.
* **`cron` shutdown of a running job.** The runner's shutdown gives a running command the entry's `GracefulTimeout` (the runner default is five minutes) to unwind after cancelling its context, then abandons it with its scope closed and reports it at warning ([`runner_command.go`](../../integrations/cron/v3/runner_command.go)); it used to wait for it without a bound when the entry set no `Timeout`. *Symptom:* a job that ignores its cancelled context no longer holds `melody:cron:run` or `--once` open past that window. *Remedy:* have the command return when its context is cancelled, or set a `GracefulTimeout` sized for its unwinding.
* **`rueidis` call timeout.** Every round trip of the [`Locker`](../../integrations/rueidis/v3/lock.go), the [`NonceGuard`](../../integrations/rueidis/v3/nonce_guard.go), the [`RedisTokenStore`](../../integrations/rueidis/v3/token_store.go) and the server-sent event backplane is bounded by a call timeout of one second by default, capping the caller's context; reads used to run unbounded and writes until the connection's write timeout. *Symptom:* a call a slow store used to answer in more than a second is refused with the deadline in the error's chain, and the context-less token store doors panic with it. *Remedy:* size the bound with `WithLockerCallTimeout`, `WithNonceGuardCallTimeout`, `WithTokenStoreCallTimeout` or `WithServerSentEventBackplaneCallTimeout`.
* **`amqp` backplane call timeout.** Each stretch of a server-sent event `Publish` over amqp, the wait for its turn and the write, is bounded by [`ServerSentEventBackplaneConfig.CallTimeout`](../../integrations/amqp/v3/server_sent_event_backplane.go), one second by default. *Symptom:* a broadcast a slow broker used to take in more than a second fails, and a write that outlives the timeout cuts an owned connection, which is redialed on the next publish. *Remedy:* set `CallTimeout` to the budget a broadcast may take.
* **`amqp` delay buckets.** The transport refuses at construction a delay bucket below one millisecond or beyond the 32-bit millisecond wire limit ([`transport.go`](../../integrations/amqp/v3/transport.go)), as it already refused a descending pair. *Symptom:* a boot declaring such a bucket panics with `amqp transport delay buckets must be between 1ms and the 32-bit millisecond wire limit`. *Remedy:* keep every bucket inside that range; a longer delay is not one the broker can hold.
* **`opentelemetry` nil middleware.** [`module.go`](../../integrations/opentelemetry/v3/module.go) refuses at boot a nil entry in `ModuleConfig.Middlewares` or `ModuleConfig.HandlerDecorators`, which it used to skip. *Symptom:* the boot panics with `opentelemetry module received a nil middleware` or `… a nil handler decorator`, typically over a constructor whose error was discarded. *Remedy:* check the constructor's error and pass only what it built.
* **`awss3` empty credential.** [`NewClient`](../../integrations/awss3/v3/provider.go) refuses an empty access key or secret key, which minio read as a request for anonymous access. *Symptom:* a boot whose credential variable is unset fails with `object storage credentials are incomplete`, where it used to start anonymously, serve reads off a public bucket and fail every write. *Remedy:* set both credentials; an application that means anonymous access builds the minio client itself.
* **`websocket` module configuration.** The [`websocket`](../../integrations/websocket/v3/module.go) module refuses, when it registers its route at boot, an empty `Path`, and `NewStreamHandler` refuses a nil `Hub` at the same moment; both used to register no route. *Symptom:* the boot panics naming the missing value, where it used to start with the stream endpoint answering 404. *Remedy:* configure the hub and the path, or do not register the module.
* **`websocket` empty topic.** A `TopicResolver` that answers an empty topic has the connection refused before the upgrade ([`handler.go`](../../integrations/websocket/v3/handler.go)); it used to be subscribed to a topic every such connection shared. *Symptom:* a client whose topic could not be extracted is refused instead of receiving the broadcasts of every other such client. *Remedy:* have the resolver answer a topic for every connection it accepts.
...
* **`outbox` lease lost mid-batch.** A lease refresh that fails no longer ends `Relay.RunOnce` at that row ([`relay.go`](../../integrations/outbox/v3/relay.go)): the claimed rows are fenced by their claim token and invisible to the new holder, so the batch is drained to its end under its claim tokens, the refresh is not tried again, and the failure is reported once the batch is done. *Symptom:* a relay that lost its lease used to leave every unreached row of the batch claimed and invisible to every replica for the whole `VisibilityTimeout`; it now finishes the batch, and the `melody:outbox:relay` loop logs the refresh failure and backs off after it. *Remedy:* none; a caller of `RunOnce` that matched the refresh error by identity reads it through `errors.Is` now, since it is wrapped with the lock name.
* **`outbox` panics in the codec or the transport.** A panic raised by `MessageCodec.Decode` or `Transport.Send` is contained per row ([`relay.go`](../../integrations/outbox/v3/relay.go)) and charged as the failure the collaborator would have returned — a decode panic dead-letters the row, a send panic takes the retry path — with the panic in `last_error` under a `panic: ` prefix and an `outbox delivery panicked` record in the log naming the row and the phase. *Symptom:* a relay process that died at one row on every claim, until the crash-poison cap dead-lettered it, now resolves that row and goes on. *Remedy:* none; a supervisor that relied on the crash to notice a poison row reads the log record or the `dead` rows instead.
* **`outbox` store claim limit.** `Store.ClaimDueMessages` ([`store.go`](../../integrations/outbox/v3/store.go)) refuses a non-positive limit by name and caps a limit above 100000 to it. *Symptom:* a direct caller of the store that passed zero, a negative or a value past the int32 range used to claim the whole table in one transaction — bun writes no `LIMIT` clause for such a value; it now gets an error carrying the limit, or a capped claim. *Remedy:* pass a positive limit.
* **`opentelemetry` metrics status.** [`metrics_middleware.go`](../../integrations/opentelemetry/v3/metrics_middleware.go) records a handler error at the client-facing status the kernel derives from it — a `404`/`403` carried as an `HttpException` graphs at its own status — instead of always `500`, and [`tracing_middleware.go`](../../integrations/opentelemetry/v3/tracing_middleware.go) sets that status on the span rather than omitting it on an errored request. *Symptom:* a route whose normal contract is a 4xx no longer reads as 100% 5xx under `http.server.request.*`, and spans for handled 4xx now carry `http.response.status_code`. *Remedy:* a dashboard or alert that treated every errored request as a server error is now more precise; only a non-http error or a 5xx exception counts as `500`.
* **`opentelemetry` and `awss3` credential redaction.** `otlp.Config` ([`tracer_provider.go`](../../integrations/opentelemetry/v3/otlp/tracer_provider.go)) and `awss3.Config` ([`provider.go`](../../integrations/awss3/v3/provider.go)) gain `String` and `Format` methods that redact their credential-bearing fields — `Headers`, and `AccessKey`/`SecretKey` — on every `fmt` verb, mirroring the `encrypt` key provider. *Symptom:* a log or error context that formatted one of these configs printed the collector token or the storage keys in the clear and now prints a redacted rendering with the safe fields intact. *Remedy:* none; if a tool parsed the old raw `%v` of these structs, read the fields directly instead.
* **`websocket` shut-down hub.** [`handler.go`](../../integrations/websocket/v3/handler.go) refuses a connection against a hub that has already shut down before the upgrade, reading the new `ServerSentEventHub.IsClosed`, instead of upgrading it to a `101` and closing it instantly with only a generic "read loop ended" debug line. *Symptom:* a client connecting during the shutdown drain gets a refusal rather than an established-then-vanished stream, and the refusal is recorded. *Remedy:* none.
* **`messagebus` in-memory transport close.** `InMemoryTransport.Close` ([`transport_in_memory.go`](../messagebus/transport_in_memory.go)) now closes the delivery channel `Receive` handed out, so a consumer ranging over it sees the end of stream; it previously closed only its internal signal and left the channel open forever. *Symptom:* a userland consumer that ranged `Receive()` without its own context cancellation, and so blocked on `Close`, now ends cleanly. *Remedy:* none; sends are serialised against the close, so no send lands on a closed channel.
* **`messagebus` route transport.** `RouteType` ([`routing.go`](../messagebus/routing.go)) refuses a nil or typed-nil transport at registration instead of dereferencing it on the dispatch path. *Symptom:* a mis-wired route now fails at registration, in the framed form `RegisterTransports` uses, rather than panicking on the first message its type routes. *Remedy:* register a non-nil transport for every routed type.
...
**Symptom.** A boot that used to succeed over a template holding one of these now fails, naming the parameter and the byte offset of the percent — never the name-shaped run or the unterminated tail, which are slices of the value (`Pa%SSword1` would have put `%SSword1` in the boot record); a registration with a padded name fails where it used to be accepted.
...
**Symptom.** A boot that declares `NewAccessControlRule("/public", "PUBLIC_ACCESS")` panics with `access control PUBLIC_ACCESS may not be declared on a raw prefix rule; use a segment prefix, exact, or regex rule`. A rule without `PUBLIC_ACCESS` is unchanged.
