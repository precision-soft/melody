...
* [`RegisterResponseListener`](../../http/cors/listener.go) — subscribes to `kernel.response` with [`ResponseListenerPriority`](../../http/cors/listener.go) (`-100`) so CORS headers reach browsers even when error handlers replace the controller response. It writes into the response and nothing else, so a handler that commits its own headers to the writer before returning — a Server-Sent Events stream, a long poll — is past its reach: a streaming route needs the `Middleware`, which writes the headers to the writer before the handler runs.
...
The router matches on the escaped path as the client sent it — `URL.RawPath` while it still unescapes to `URL.Path`, else `URL.EscapedPath()`, which alone would re-spell the decoded path whenever the raw spelling carries a byte it does not keep, such as `{` or raw UTF-8 — and unescapes each segment **after** splitting on the separators the client actually sent. An encoded slash therefore stays inside the value where the client put it: `/admin%2Fusers` is one segment naming a resource called `admin/users`, not two segments reaching the `/admin/users` handler. Every other consumer of the path that takes a decision on it reads the same spelling through [`RequestPathAsRouted`](../../http/router_utility.go) — the canonical-path guard in the kernel, the access-control matcher, the firewall path matcher, the static file server and its exclusion list, the compression exclusion list and the internal-auth endpoint binding — so a rule and a route cannot disagree about which resource an encoded separator names; the kernel's journals still print the decoded `URL.Path`, while the static file server's warnings name the spelling it resolved from. The spelling is not the router's identity to the letter: a segment that unescapes to a literal `%2F` (`%252F`) and one carrying an encoded separator (`%2F`) both read `%2F` here while the router keeps them apart, so a rule spelled with `%2F` names both — a limit written on `RequestPathAsRouted`, kept until a major may change the spelling of rules. Matching the decoded path made `%2F` a segment separator, so one url reached a handler that a proxy or WAF rule written against the raw request line had never seen — which is also why [`GeneratePath`](../../http/url_generator.go) refuses a `/` inside a parameter value.

**A handler mounted in front of the kernel that rewrites the path must rewrite both fields.** `URL.Path` is the decoded path and `URL.RawPath` the spelling the client sent; a rewrite of `Path` alone leaves a `RawPath` that no longer unescapes to it, and the kernel refuses such a request with a 400, unmatched, before it is authorized or handled — the spelling the client sent can no longer be read, and the decoded path would read an encoded separator as a separator. `nethttp.StripPrefix` rewrites both. A handler of its own applies the same edit to `RawPath` whenever it is set, and never clears it: the kernel cannot tell a cleared `RawPath` from a request that carried no escape, so `/api/admin%2Fusers` would reach the `/admin/users` handler:

```go
request.URL.Path = strings.TrimPrefix(request.URL.Path, "/api")
request.URL.RawPath = strings.TrimPrefix(request.URL.RawPath, "/api")
```
...
A requirement declared on a parameter is validated against the value that is actually emitted, catch-all remainders included, so generation and matching agree. A catch-all remainder keeps an encoded separator inside the segment the client put it in: the request path is split on the separators actually sent and each segment unescaped on its own, so `/files/a%2Fb/c` binds `a%2Fb/c` and stays distinguishable from `/files/a/b/c`, which binds `a/b/c` as before. A handler that reassembles a storage key or a proxy target from the remainder unescapes it (`url.PathUnescape`) when it wants the literal slash.
...
The server's `WriteTimeout` is armed by `net/http` ONCE, absolute from the moment the request line was read, so a stream that lives longer than it loses the first event published after it: every write fails from then on, the handler learns it only when that event arrives, and the connection is cut under a client that still believes it open. [`WithWriteBudget`](../../http/server_sent_event.go) makes the writer re-arm the deadline before every frame, the budget from the moment the frame is written, through a `ResponseController` that reaches the connection through whatever wrapped the writer — the kernel's recording writer included. A stream that keeps writing keeps living; a client that stops reading is cut one budget after the first frame the socket can no longer buffer — which is not the first frame it did not read: small frames at a slow cadence sit in the kernel buffers for a long time, so a stalled client holds its connection until they fill, and an absolute bound on a stream's life is the handler's to declare, not the deadline's. The budget to hand in is the server's own `WriteTimeout`, read off `request.Context().Value(nethttp.ServerContextKey)`, and an idle stream needs a keepalive ticker beside the events so it re-arms too; zero leaves the deadline as the server armed it, and a writer the controller cannot reach the connection through takes every frame as it did without a budget.
...
The example application wires an `ServerSentEventHub`, an `/events/stream` Server-Sent Events endpoint (`handler/event/stream_handler.go`), and an `/events/publish` endpoint (`handler/event/publish_handler.go`) that dispatches a message through the bus to a handler which broadcasts to the hub.
...
The kernel enforces the same rule for the application's own routes: a request path that folds to a different spelling is refused with a `400` before it is authorized, handled or served by any route, so the router, the firewall matchers and the access control never disagree about which resource a request names. A trailing slash is not a fold and is served; a target that does not begin with `/` — the asterisk-form of `OPTIONS`, an authority-form `CONNECT` — is left to the router.
...
* Route parameter requirements:
    ...
    * [`FilterRouteManifestByZone(RouteManifest, zone string) (RouteManifest, error)`](../../http/route_manifest.go)
...
* Compression:
    ...
    * Honors `Accept-Encoding` q-values (RFC 7231) — a header of more than sixty-four members is read as unparsable and the response is not compressed, since the members past the cap can carry the refusal the members before it do not, and a legitimate header never reaches the cap —, emits `Vary: Accept-Encoding`, and streams the gzip output through [`io.Pipe`](https://pkg.go.dev/io#Pipe) so response bodies never fully land in memory.
