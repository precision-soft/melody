...
**A handler mounted in front of the kernel that rewrites the path must rewrite both fields.** `URL.Path` is the decoded path and `URL.RawPath` the spelling the client sent; a rewrite of `Path` alone leaves a `RawPath` that no longer unescapes to it, and the kernel refuses such a request with a 400 after the route is matched and before it is authorized or handled — the spelling the client sent can no longer be read, and the decoded path would read an encoded separator as a separator. `nethttp.StripPrefix` rewrites both. A handler of its own does the same, or clears `RawPath` when it rewrites to a path that carries no escape:
