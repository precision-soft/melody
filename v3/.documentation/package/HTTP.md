...
The router matches on the escaped path — `request.URL.EscapedPath()` in [`ServeHttp`](../../http/kernel.go) — and unescapes each segment **after** splitting on the separators the client actually sent. An encoded slash therefore stays inside the value where the client put it: `/admin%2Fusers` is one segment naming a resource called `admin/users`, not two segments reaching the `/admin/users` handler. Every other consumer of the path reads the same spelling — the canonical-path guard in the kernel, the access-control matcher and the firewall path matcher, through [`RequestPathAsRouted`](../../http/router_utility.go) — so a rule and a route cannot disagree about which resource an encoded separator names. Matching the decoded path made `%2F` a segment separator, so one url reached a handler that a proxy or WAF rule written against the raw request line had never seen — which is also why [`GeneratePath`](../../http/url_generator.go) refuses a `/` inside a parameter value.
