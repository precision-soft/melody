...
```go
app.RegisterCliCommand(wiring.NewGenerateCommand(config.NewWiringBindSet()))
```

Flags: `--out <path>` (write to a file relative to the project directory, with the report lines on the command's writer beside the `wiring written to` line; prints the source alone to stdout when empty, and the report lines then go to the application's journal one record per line — the lines that state a fact of a scan that worked as information (the counts of what was registered, the vendor trees stepped over), every line naming lost coverage (a skipped constructor, an unmatched bind or exclude, a file the build constraints left out, bind targets unchecked) as a warning, so a journal read at the usual production threshold keeps them; the reach of the global binds is a warning too, since a global bind reaches every constructor declaring an argument of that name and the line is where an operator learns how many that is, but it is ONE record for the whole report rather than one per bind — its message names how many binds reached and its context carries a line apiece — so a clean generation raises exactly one warning however many binds it resolved, and none at all without them; to the emergency journal on stderr when the application journals to stdout itself, since that stream is the source; a logger the application substituted for the container's cannot be read, so an application journaling to stdout through its own logger keeps the report out of the source by giving `--out`), `--quiet` (default `true`: the run banner is not written around the printed source; `--quiet=false` brings it back), `--package <name>` (default `config`), `--function <name>` (default `RegisterGeneratedServices`), `--scoped-function <name>` (default: the registration function name with `Scoped` appended; only emitted when a constructor carries `//melody:scoped`), `--strict`, `--tags <a,b>` (build tags the target binary carries), `--report-vendor` (name the vendor directories the scan stepped over), `--report-excluded` (name the build-excluded files that hold a constructor candidate). The committed generated file is what the application registers, so it is regenerated whenever a scanned constructor changes and kept under version control.
...
- The scanned directories must not include the package the generated file is written into, or the generated file would import its own package — the command refuses an `--out` inside a scanned directory for exactly this reason, reading the containment on path components (a directory named `..hidden` inside the scanned one is inside it) and refusing an output path it cannot relate to a scanned directory rather than reading the failed relation as outside.
