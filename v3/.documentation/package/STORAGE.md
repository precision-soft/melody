...
- [`LocalStorage`](../../storage/local.go) sanitizes keys against path traversal (a key resolving outside the base directory is rejected, including via a symlink — a leaf symlink is refused, and every operation runs through an `os.OpenRoot`-pinned `*os.Root` that checks each path component against symlink escape and never traverses a link) and does not support `PresignedUrl` (it returns an error).
- `Put` takes the content size; pass `-1` when the size is unknown (S3 backends stream it, `LocalStorage` skips its length check). When a non-negative size is given, `LocalStorage` enforces it — a reader that does not yield exactly that many bytes returns an error. A failed `Put` leaves no object behind: `LocalStorage` removes the partially-written file on any write failure, including a size mismatch and a source reader that errors mid-stream (so a streaming `-1` upload whose reader fails does not leave a truncated object), matching the S3 backend's atomic-on-failure behaviour. What the in-process error paths cannot clean is a crash mid-write: the temp object it leaves behind is swept by a later `Put` of the same key, once its modification time is at least an hour old. Temp objects live in a reserved hidden namespace beside the key, `.melody-storage-<sha256 of the key's leaf>.<16 hex>.tmp`, which fits one path component whatever the leaf's length; only regular files of exactly that shape for that key are ever swept, so a concurrent writer's live temp, another key's temp and an ordinary key that reads like a temp name, such as `report.tmp-0123456789abcdef`, are never touched. A key whose leaf is spelled exactly like a temp object is refused at every door, since a later `Put` of the key its digest names would sweep it. A temp object a crash left under the naming of `v3.13.0`, `<leaf>.tmp-<16 hex>`, is not of the reserved shape and is never swept; remove it by hand.
