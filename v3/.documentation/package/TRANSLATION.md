...
- A `plural` argument that is present but non-numeric — a `nil`, a string — resolves the `other` branch and substitutes `#` with an empty string, rather than being treated as the number zero; an ABSENT argument renders the visible placeholder, as the next bullet says. This differs from ICU/MessageFormat implementations that default a missing argument to `0` — supply the argument explicitly if you depend on a numeric category.
- A placeholder whose parameter is absent renders as the visible placeholder itself (`Hello, {name}!`) — the plain placeholder, and a `plural` or `select` argument alike (`{count}`, `{gender}`) — so a renamed parameter key is observable in the output instead of silently deleting the value or the count; a parameter present with an explicit `nil` renders as the empty string, or resolves the `other` branch of a `plural`, because the caller said so. A `select` argument whose keyword matches no branch resolves the `other` branch, and an empty string when no `other` exists.
- `MapCatalog.Add` is a boot-time builder with no synchronization: build catalogs, hand them to `NewManager`, and stop writing — an `Add` concurrent with any `Trans` is a data race. The `Manager` itself is immutable after construction and safe for concurrent use.
- Several catalogs per locale: [`NewManager`](../../translation/manager.go) keeps every catalog of a locale, in the order given, and a lookup asks them in that order until one answers, so a locale assembled from several sources — catalogs wired by hand, or loaded from two directories; the json loader itself merges the files of one directory into one catalog per locale — loses nothing, and a message two catalogs hold is answered by the earlier one. A catalog whose `Locale()` is empty is refused at construction, as a nil catalog is: the locale chain never asks for the empty locale.
- `JsonDirectoryLoader` expects files named `<domain>.<locale>.json` containing a flat `{messageId: message}` object; a `.json` file whose name does not parse that way fails the load naming the file, so a naming typo cannot ship a translator answering raw message ids (directories and non-`.json` files are simply ignored). There is no Manager bridge helper; load file-based catalogs explicitly and pass them into the constructor: `catalogs, err := translation.NewJsonDirectoryLoader(dir).Load()` then `translation.NewManager("en", []string{"en"}, catalogs...)`.
