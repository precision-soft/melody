...
- A PLAIN placeholder whose parameter is absent renders as the visible placeholder itself (`Hello, {name}!`), so a renamed parameter key is observable in the output instead of silently deleting the value; a parameter present with an explicit `nil` renders as the empty string, because the caller said so. A `select` argument that is missing resolves the `other` branch, and an empty string when no `other` exists.
