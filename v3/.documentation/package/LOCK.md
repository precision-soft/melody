...
The threshold covers the one case the lease clock cannot see: a gate configured with a cadence far denser than its lease would otherwise keep leader work running for the whole lease against a store that has plainly gone. At the default cadence of half the ttl it is unreachable — three renewals already outlast the lease — so an ordinary deployment is governed by the lease alone. Set it negative to remove it entirely and leave only the lease clock.
