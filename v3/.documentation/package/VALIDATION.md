...
- `greaterThan`/`lessThan` operate on numeric fields only and reject a non-numeric value with `value must be numeric`; a floating-point `NaN` is rejected rather than silently passing the bound (`NaN` compares false against every threshold). The bound is an integer in its entirety: a fractional, suffixed or otherwise non-integer bound (`value=1.5`, `value=1e3`) fails the constraint's construction and the rule reports `invalidRuleSyntax` on every value it reaches. The [`openapi`](OPENAPI.md) mirror reads every numeric bound with the same all-or-nothing parse, so the published spec refuses exactly the tags the server refuses — an equivalence held by an executable proof, [`openapi/schema_test.go`](../../openapi/schema_test.go), which runs the generated document and this validator against the same values rather than trusting either side's prose. For a **float-typed** field the comparison is exact at every magnitude: the bound is never converted to `float64`, whose rounding above 2^53 misjudged the values adjacent to an unrepresentable bound (a field holding `9007199254740996.0` was refused by `greaterThan` with bound `9007199254740995`, which rounds to `9007199254740996`). The frozen majors keep the converted comparison with the rounding documented; here the value a field holds is judged against the declared number, on every branch.
...
- A pointer several fields share is validated under **every** path that reaches it, and walked once per depth: the cycle set is scoped to the current path — only an ancestor still on the path can close a cycle — and a memo keyed on the pointer, its type and the depth it is reached at answers a later path with the errors of the first walk re-spelled under the new path. A payload decoded from json shares nothing; the memo is for a value the application built, which used to cost 2^N walks on N levels of two-way sharing through pointer fields — the memo keys pointers, so a value shared through slices or maps alone is still walked once per path. A genuinely cyclic value reports the cycle once, on the path that closes it; a node that is both shared and on a cycle is memoized as the first path saw it, so the paths its report is spelled under depend on which field reached it first. An error a constraint answers under a field of its own, or as an error type of its own, is kept verbatim under every path — "under the walked path" is read in the walk's own grammar (the path, a member `.x` or an element `[i]` of it), so an own field that merely begins with the path's text is verbatim too. The node an own-typed error was raised under is not memoized, since such an error can only be answered under a later path by being re-spelled into the package's type: that node is walked again per path, and the 2^N cost the memo removed returns for exactly that shape — a constraint answering its own error type on a pointer many fields share.
- `NewRegex("")` fails closed the way a pattern that does not compile does: the constraint's `Error` names the empty pattern and `Validate` refuses every non-empty value with `invalidPattern`, so a rule declared with an empty pattern never validates everything in silence; the tag door refuses `regex=` before construction.
