...
- An `Accept` header of more than sixty-four members (or a member of more than sixty-four parameters) is refused whole with [`ErrNotAcceptable`](../../serializer/serializer_manager.go): the split is bounded so a megabyte of separators cannot become a member per byte, and the members past the cap can carry a refusal the members before it do not, so the negotiation does not score the part it read. The same header makes `PrefersHtml` answer false and the compression middleware serve identity. A legitimate header never reaches the cap.
