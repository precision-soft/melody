...
- Both storages refuse `Load`, `Save` and `Delete` after `Close`, and both treat the expiry instant itself as lapsed. **They keep the expiry on different clocks, deliberately.** `InMemoryStorage` keeps the expiry as an in-process `time.Time`, which carries Go's monotonic reading: an NTP step neither lapses nor resurrects a session, and no entry survives a restart anyway. `FileStorage` persists `expiresAt` as unix nanoseconds — the wall reading is the only one that can survive a restart — so its expiry follows every step of the wall clock. An application that moves between the two keeps the same boundary (the instant itself is lapsed on both) but trades monotonic stability for restart durability. Every instant comes from the injected clock: the `…WithClock` constructors take one, the plain constructors read the system clock, and the framework-wired default storage runs on the kernel's clock — so a frozen clock in a test moves session time deterministically. Neither serialises anything across processes: `Storage.Save` is a blind upsert, so two concurrent requests writing different keys of the same session end with whichever wrote last, and a deployment running more than one node needs a storage of its own that makes those writes conditional.
...
- [`session.NewInMemoryStorageWithClock(cleanupInterval, clockInstance)`](../../session/in_memory_storage.go) — every expiry instant and the sweep's tick from the injected clock
- [`session.NewFileStorageFromPath(path)`](../../session/file_storage.go)
- [`session.NewFileStorageFromPathWithClock(path, clockInstance)`](../../session/file_storage.go) / [`session.NewFileStorageFromFileWithClock(fileInstance, clockInstance)`](../../session/file_storage.go) — the same doors with the clock injected
