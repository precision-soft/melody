...
`debug:container` also renders the container's teardown plan, read through the `TeardownPlan()` door of the concrete container (see [CONTAINER](CONTAINER.md)): the `TEARDOWN (SEQUENTIAL)` block — `TEARDOWN (DEPENDENCY WAVES)` once the application armed the parallel teardown — lists every built service with its `wave`, the services it is `closed before`, the services `closed after` it, an `ordering` that reads `proved` when the graph relates the service to anything in either direction and `none` when nothing does, and a `group` — the serial group of the wave the service is closed one after the other WITH, empty for a service in none: two services seen holding each other, or a ring of them, carry no edge and no ordering, but a wave closes them one at a time rather than together, and the figure is what says so. On the `TEARDOWN (SEQUENTIAL)` block the rows are printed in the order the sequential teardown closes them and the `wave` column is what arming would run; the two agree only under waves, since the sequential teardown honours every edge but closes creation latest-first among what is free, so a wave-zero row can stand below a wave-one row it has no relation to. The `none` rows are the ones to read before arming the waves: under waves a service nothing orders closes beside everything else in its wave. The block renders the same on the default listing, on the `--build` sweep and on the single-service door, kept to the window the listing applied while the wave index stays the whole plan's; the json document carries the same answer as a `teardown` object — `wave`, `node`, `closedBefore`, `closedAfter`, `ordering`, `group` — beside each built service, and omits the key on a registration the plan does not list (never built, or scoped — a scoped registration allowed to share its name with a built container service carries no key either, on the `--build` sweep too, whose items carry their `lifetime` so the two rows of one name are told apart), so `jq '.data.items[] | select(.teardown.ordering == "none")'` names the services nothing orders. The block is windowed on the container services the listing shows: a window holding only such a scoped twin shows no row for it, and neither does the single-service door asked by the twin name, which answers the scoped registration a console process reaches. An `ordering` of `cycle` marks a service ON a ring the drain could not open — not the pure dependencies of its members, which the drain releases once the ring is closed and closes after it, in the order their edges prove: a ring is closed one service at a time, in creation order, the teardown reports its members, and a plan that shows one is one to read before arming. The table prints a type alias by the type's own string, `type:*pkg.T`. A name the plan folded onto another — two names handed one pointer, or a service registered under a name and resolved through its type — answers with the entry of the node it was collapsed onto: `node` names that node, the table row prints it `service:a (also service:b)`, and a listing windowed on either name shows the row. A container that does not carry the door renders no block.
