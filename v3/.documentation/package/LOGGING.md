...
- **`LevelReporter` is an optional capability, and its absence means "enabled".** A logger may implement [`loggingcontract.LevelReporter`](../../logging/contract/logger.go) (`Enabled(level Level) bool`) to say, before being handed anything, whether a record at that level would survive its threshold. It exists for the callers that *build* what they log — the event dispatcher assembles a context map per dispatch and resolves a listener's name through reflection per listener per dispatch, all of it discarded unread by a journal at error level. Ask it through [`logging.LevelEnabled(logger, level)`](../../logging/logger.go), never through your own type assertion: the fallback for a logger that does not implement it is **true**, and a site that spelled that the other way would stop recording against every logger that does not carry the capability. `jsonLogger` implements it against its own threshold (and reports nothing enabled once closed), `nopLogger` reports nothing enabled at all, and
  `requestLogger` forwards to the base it decorates — which is what makes it worth anything on the request path, since the decorator is what every handler and listener actually holds. Use it only where the answer saves work that is otherwise thrown away; a record whose arguments already exist costs nothing to hand over, and gating it would only add a second place where a level decision is made. `loggingcontract.Logger` is untouched, so an existing logger keeps working unchanged.
- **A logger that can be closed should also answer `Closed() bool`.** The container's teardown closes the logger service when it implements `io.Closer`, and a process that dies after that teardown still owes its last record: the exit path asks the container's logger `Closed()` and, on true, writes the record through a file logger of its own on the configured destination instead (see [`application.go`](../../application/application.go)). A logger that implements `Close` without `Closed` gives no such answer, so the exit path hands the final record to a logger that has stopped writing, and the record is lost. `jsonLogger` implements both, and `requestLogger` forwards `Closed()` to the base it decorates.
- Context keys should be camelCase. This is relied on across Melody (for example `processId`, `requestId`).
- **The file journal reopens on `SIGHUP`, which is what rename-based rotation needs.** The default configuration logs to a file (`MELODY_LOG_PATH` defaults to `%kernel.logs_dir%/%kernel.environment%.log`), and the container opens it through [`logging.NewReopenableFileWriter`](../../logging/reopenable_file_writer.go), armed on `SIGHUP`: a logrotate in its default rename-and-create mode moves the journal out from under the descriptor — writes into a renamed or deleted-but-open inode keep succeeding, so nothing errors while the fresh file stays empty — and the conventional `postrotate` `SIGHUP` now makes the writer open the path again and point every later record at the fresh file. A reopen that fails keeps the current descriptor writing and says so once on stderr. `copytruncate` remains a valid alternative where no signal can be delivered: the descriptor is opened with `O_APPEND`, whose per-write offset makes truncation-in-place safe, at the price of the small copy
  window that mode always carries. The stable majors keep the single-descriptor shape and the `copytruncate` prescription; the reopen door is this major's. The writer belongs to the logger, so the container's teardown closes it, and `Close` disarms the signal watcher before surrendering the descriptor.
- [`NewJsonLogger`](../../logging/json_logger.go) serializes writes through an internal mutex so concurrent calls produce cleanly-separated JSON lines on the underlying writer, stamped in UTC with the nanosecond field written to its full width, so the stamps sort as text into the order they were written. The stamp is taken **inside** that mutex, which is what makes the order of the stamps the order of the writes: taken above the lock it said when the record was formed, and the work sitting between the stamp and the write put records on the file in an order the stamps did not describe. The fixed width is the other half of the same guarantee — `RFC3339Nano` trims trailing zeros, so a record landing on a whole second rendered shorter and sorted ahead of every fractional record of that same second. What runs under the lock is assembly of already-encoded values and the write itself; the normalization of the caller's context **and its encoding** both stay outside it, because both run the caller's own code — a `MarshalJSON`, a `String` — which is unbounded work this logger does not own and which may itself log through this very logger. Errors in the context — top-level or nested inside the map and slice shapes the framework produces — render as their messages, except
  an error that also implements `json.Marshaler`, which is handed to the encoder for a structural rendering (`validation.ValidationErrors` opts in, so the log says what the response body says); a record whose context cannot be marshaled falls back to a line that keeps the context as rendered text beside the marshal error. A record whose level is not one of the five known ones is weighed at error priority — the unknown level is the case that least deserves silence — with the raw level as its label. `Close` leaves the process console open, recognized by identity (`os.Stdout`/`os.Stderr` themselves) — a file the application opened on a console-looking path is really closed; `Closed()` answers lock-free, so the process-boundary exit handler is never queued behind a stalled write. The level-label map is copied at construction, so a reference the caller keeps cannot race the lock-free label reads. A failed write — a full disk, a read-only mount, a vanished volume — is echoed once for the
  life of the logger on stderr, written there directly rather than through `EmergencyLogger`, which is itself a json logger and would re-enter the write that just failed; the echo is skipped when the logger's own output already is stderr. Once, because a logger writing into a full disk fails on every record it is handed.
...
- [`NewJsonLoggerWithClock(output io.Writer, minLevel loggingcontract.Level, labels loggingcontract.LevelLabels, clockInstance clockcontract.Clock)`](../../logging/json_logger.go) — stamps every record from the given clock; the constructors without one read the process clock, because they also build the last-resort loggers, where no injected clock exists to be wrong
- [`NewReopenableFileWriter(path string) (*ReopenableFileWriter, error)`](../../logging/reopenable_file_writer.go) — the journal writer rename-based rotation needs: [`Reopen()`](../../logging/reopenable_file_writer.go) opens the path fresh and swaps the descriptor, [`ArmReopenOnSignal(signals ...os.Signal)`](../../logging/reopenable_file_writer.go) does it on each delivered signal — `SIGHUP` is the conventional choice, and the container arms exactly that — and [`Close()`](../../logging/reopenable_file_writer.go) disarms the watcher before surrendering the descriptor
...
- [`LogOnRecoverAndExitAfter(logger loggingcontract.Logger, recovered any, exitCode int, beforeExitBudget time.Duration, beforeExit func(beforeExitContext context.Context))`](../../logging/recover.go) — the exit handler, with the teardown hook running under the budget its caller declares. The panic path and the clean return release the same brokers, pools and tracer providers, so a budget honoured on one and ignored on the other would leave every command that exits non-zero closing under a figure its operator had already replaced
- [`RunShieldedStep(stepName string, step func(stepContext context.Context)) bool`](../../logging/recover.go) — the exit handler's own shield, offered to the one other caller that stands between a process and its end: it contains a panic inside the step and bounds how long the step may take, answering whether it finished. An abandoned step keeps running on its goroutine, so nothing it writes may be read by a caller that was told it did not finish
- [`RunShieldedStepWithin(budget time.Duration, stepName string, step func(stepContext context.Context)) bool`](../../logging/recover.go) — the same shield under a budget the caller declares, for the process shutting down cleanly, whose services carry close budgets this package cannot see. The step is handed a context carrying a deadline of HALF that budget, and the shield abandons it at the whole of it: a step told to finish at the very instant the shield gives up returns after the timer has already fired, so it is abandoned every time and what it was writing reaches nobody. A non-positive budget installs no deadline at all, on the context or on the shield, and the step is waited out; the exit handler falls back to the package's own figure when its caller declares none, because this package cannot read the configuration — the logger it builds is what the configuration is loaded through
