...
/* the embedded time.Time promotes the codec for the whole value, so the body is an RFC 3339 string: encoding/json fills the embedded time and never the sibling, and enforcing the sibling's constraint would reject every body the spec describes */
