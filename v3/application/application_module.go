...
func (instance *Application) bootModulesPostConfigurationResolve() {
    ...
    /* one loop per hook, like every phase above: each hook runs across every module before the next hook begins, which is the granularity the contracts document — a module may rely on every sibling's listeners existing before any middleware registers, and folding hooks into one sweep would silently change that rule for exactly these five */
    for _, moduleInstance := range instance.modules {
