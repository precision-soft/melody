...
- `http/sse.go` — `SseWriter.Send` strips `CR`/`LF` from the `Id` and `Event` fields so a caller-supplied value can no longer inject extra SSE fields or events (`Data` was already split safely into multiple `data:` lines).
