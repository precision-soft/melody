...
- `mailer/` — pluggable email sending. `mailer/contract` (`Mailer`, `Transport`, `Message`, `Address`, `Attachment`); `NewManager(transport)` validates sender and recipients; `NewSmtpTransport(SmtpConfig)` (stdlib `net/smtp`, STARTTLS when the server offers it) and `NewInMemoryTransport()` for tests. `RenderMessage` builds RFC 5322 / MIME messages: quoted-printable bodies (kept within the SMTP 998-character line limit), `multipart/alternative` for text+HTML, `multipart/mixed` with base64-encoded attachments, CRLF-stripped headers/addresses, and reserved-header filtering so callers cannot override the structural headers.
- `storage/` — object-storage abstraction. `storage/contract.Storage` (`Put`/`Get`/`Delete`/`Exists`/`PresignedUrl`); a `NewLocalStorage(baseDirectory)` filesystem backend with base-directory escape protection. An S3-compatible backend ships in `integrations/objectstorage/v3`.
- `http/sse.go`, `http/sse_hub.go` — Server-Sent Events. `NewSseWriter(w)` streams `text/event-stream` (sets the headers and flushes after every `Send`); `NewSseHub()` is a topic-keyed fan-out (`Subscribe`/`Unsubscribe`/`Broadcast`) with **at-most-once** delivery — a full subscriber buffer drops the event, and `DroppedEventCount()` exposes the cumulative loss as a metric.
