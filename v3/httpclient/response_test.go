...
/* the three accessors carry everything about a response except its body and its status code. Status is the reason phrase a caller logs, Headers is what a caller reads a content type or a rate-limit budget out of, and Request is the only way back to the url the exchange actually reached after the base url and the query are folded in — a redirect chain ends somewhere the caller never spelled, and this is where it says so. */
...
/* Headers hands back the map it is constructed with, without a copy, and the buffered path constructs it with the transport's own header map while the STREAMING path clones. This test records the behaviour as it stands, not as it ought to be: a caller that mutates what Headers returns changes what every later reader of the same response sees, where the sibling API does not. */
