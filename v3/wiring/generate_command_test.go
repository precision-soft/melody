...
func TestGenerateCommand_TheStdoutSourceIsNotWrappedInTheBanner(t *testing.T) {
    ...
    /* the report still precedes the source on the one writer in stdout mode — that is the report's own question, not the banner's; what the banner added was the escape sequence and the frame before the first report line */
    if false == strings.HasPrefix(written, "registered 1 constructors\n// Code generated by melody:wiring:generate. DO NOT EDIT.") {
        t.Fatalf("expected the report and the generated source alone on stdout, got %q", written[:min(len(written), 80)])
    }
