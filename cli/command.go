    ...
    commandContext.Commands = append(
        ...
        &clicontract.CommandContext{
            ...
            Action: func(ctx context.Context, commandContext *clicontract.CommandContext) error {
                ...
                /* a command that returns its error through a concrete typed pointer hands over a non-nil interface around a nil value: read as a failure it reaches Error() on a nil receiver where the failure is printed. The same normalization guards the scope's Close result, which crosses the substitutable runtime contract. */
