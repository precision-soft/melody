...
func RegisterKernelResponseNormalizerListener(eventDispatcher eventcontract.EventDispatcher) {
    eventDispatcher.AddListener(
        ...
        func(runtimeInstance runtimecontract.Runtime, eventValue eventcontract.Event) error {
            ...
            /* the kernel replaces a handler's nil with an empty 204 before it dispatches the event, so nil here can only be a listener that cleared the response, which writeResponse answers with the same empty 204; the normalizer does not decide a second time what no response means. */
