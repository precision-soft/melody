...
/* the constraint lists are copied at construction, so a registrant reusing its slice across definitions, or mutating it after registration, cannot rewrite the ordering constraints the pipeline was registered under. */
