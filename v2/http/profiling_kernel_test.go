...
/* The request is the application's and Attributes() returns an interface, so a nil pointer of the application's own bag type arrives as a non-nil interface. Read with a bare comparison it would be taken for a live bag and Get would dereference the nil receiver inside a response listener, where no recover covers it. */
