...
/* normalizedEventResponse stores the nil a listener means: a nil pointer of a Response implementation arrives as a non-nil interface, every reader asks `nil == Response()`, and the writer would dereference it on the first Headers() call, a second panic on the recovery path. */
