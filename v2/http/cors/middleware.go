...
func Middleware(service *Service) httpcontract.Middleware {
    ...
    return func(next httpcontract.Handler) httpcontract.Handler {
        return func(runtimeInstance runtimecontract.Runtime, writer nethttp.ResponseWriter, request httpcontract.Request) (httpcontract.Response, error) {
            ...
            /* a streaming handler commits its headers straight to the writer and returns a response the write path discards, so the cross-origin headers are applied to the writer before the handler runs: Vary on every path, the allow-origin headers when the origin is allowed. For an ordinary handler the write path replaces them with the response's own copy. */
