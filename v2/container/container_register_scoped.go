...
/* RegisterScoped is Register's counterpart for a service whose lifetime is one scope. It takes a ScopedRegistrar, and the two registrar interfaces share no method, so handing a registrar to the wrong one of the two functions does not compile. */
