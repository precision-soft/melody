...
/* signal.NotifyContext keeps SIGINT and SIGTERM registered after the first signal cancels the context, so on its own a second Ctrl+C during a hung shutdown does nothing; the watcher exists to force the exit, with the conventional 128+signal code. */
