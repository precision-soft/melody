...
- documentation: the released `v2.13.0` section carries its classification markers and its document pointers again. Compressing that section to the first sentence of every entry, for the 125,000 character cap on a GitHub release body, dropped every marker that sat in a later sentence — 25 Breaking, 162 Behavioural change, one Operational note and one Breaking at source — together with the six references to `UPGRADE.md` and the three to `SECURITY.md`, so a reader of the release could not tell which of its 628 entries breaks them; one entry was cut inside a code span as well, where the splitter read the ellipsis of `debug:container ... -vvv` as the end of a sentence. Every marker is back as the bare tag this tree already writes, every pointer as the sentence that carries it, and the truncated entry as its whole first sentence
...
- documentation: the `Unreleased` section of `.documentation/UPGRADE.md` names the release its sections shipped in. The sentence introducing them said they were the changes "currently sitting in the `[Unreleased]` block" of this changelog, which stopped being true when `v2.13.0` went out on 2026-08-18: that block now holds a different set of patch-level entries, none of which asks the upgrader for an action, so a reader following the pointer landed on the wrong list. The heading itself stays `Unreleased`, because this guide promotes at a MAJOR boundary — `v3/.documentation/UPGRADE.md` carries `v3.0.0` and no minor heading despite thirteen minor releases — and the sentence now says so
...
- documentation: four passages that resolved into another major's tree are re-pointed at this one. The rueidis readme linked the cache backend contract of v1 and its registration example imported six core packages of v1 into a module whose `go.mod` requires this major and not that one; `UPGRADE.md` claimed the example's mysql opt-out mirrors a `PGSQL_INSECURE` switch, whose only occurrence in this major was the sentence itself; and `DOCUMENTATION.md` called `../README.md` the repository root, which from this major's `.documentation/` is this module's own readme.
...
- documentation: the upgrade guide of this major catches up with the one the first major carries, and the two are now the same document.
- documentation: three documents of this major catch up with the ones the first major already carried. `DOCUMENTATION.md` gains the archetype-free documents (`CHANGELOG.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, the GitHub templates and the per-integration `README.md` files), the naming rule for the GitHub issue templates and for where a package document may live, the note that the command-surface documents title their end-of-document listing "Exported API", and the caveat that the upgrade guide covers releases made since it was introduced. `UPGRADE.md` drops the compile-level note announcing that `session/contract.Session` gained `SetShared`: no major declares that method — the change was reverted before release — so the note described a breaking change an upgrader could neither see nor act on, and the neighbouring documentation check could not report it, since that check asks whether a symbol a major ships is documented and never whether a documented symbol exists.
