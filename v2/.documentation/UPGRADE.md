...
Public regex rules must anchor every alternative to the beginning of the path. Replace `^/public|/status` with `^(?:/public|/status)(/|$)`. Multiline `(?m)^` is not a path-start anchor. Invalid public configurations fail at construction.
