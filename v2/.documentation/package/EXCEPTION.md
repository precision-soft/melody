...
- Every utility treats a typed nil — a non-nil `error` interface holding a nil concrete pointer — as the nil it means: `LogContext`, `FromError` and its variants, `MarkLogged` and both cause-chain builders neither panic on one nor hand one onward. The `Unwrap` of `Error` and of `HttpException` answers nil on a nil receiver, because `errors.Is` and `errors.As` call it on every link of a chain and a typed-nil link — `FromError(nil)` stored as another error's cause — is reached before any caller's guard; and the `From*` constructors render the message under the same recover `LogContext` uses, because they run inside the kernel's recovery defers, where an `Error()` that panics would raise a second panic through the defer reporting the first.
