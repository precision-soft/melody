...
Runtime mode and process role are determined by [`ParseRuntimeFlagsWithRole`](../../application/cli.go), which is what [`NewApplication`](../../application/application_new.go) calls; [`ParseRuntimeFlags`](../../application/cli.go) is the older door and delegates to it with the all-roles default.

- `--mode=http` or `--mode=cli` (also `-mode=...`)
- When no explicit mode is provided, non-runtime arguments imply CLI mode; otherwise the configured default mode stands.
- An explicitly supplied but **empty** `--mode=` — an unset environment variable expanded into the command line, or a bare `--mode` that cannot consume a dash-leading next token — does not fall back to the default. It fails closed with a panic naming the value, because the alternative is a process silently serving in whichever mode the default happens to be.
- `--role=<role>` (also `-role=...`) is the second runtime flag: it selects the process role, defaulting to the configured one. Both flags are runtime flags and must precede the cli command — a `--mode` placed after the command belongs to that command's own flag set.
...
A panic **during boot** exits fail-fast, with the container teardown run between the record and the exit: `Boot`'s own recover handler logs the record first, through a logger the teardown has not touched, then closes the container and exits. That close is unconditional: it probes `IsClosed` first, but only to decide whether a teardown failure is reported as this call's discovery rather than as a memoized one somebody else already had. What costs nothing when the boot died early is the nil-kernel check at the top of `close` — an application whose kernel was never assembled returns there and never reaches a container. Connections modules opened before the failing boot line — database pools, redis clients — therefore get their `Close`, bounded by the same per-step budget the exit path applies to everything else.
