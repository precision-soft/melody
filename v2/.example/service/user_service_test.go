...
func TestUpdateRefusalPreservesStoredUser(t *testing.T) {
    userService := newUserServiceUnderTest(t)
    user, found, err := userService.userRepository.FindById(context.Background(), "user-2")
    ...
    serviceContainer := melodycontainer.NewContainer()
    runtimeInstance := melodyruntime.New(context.Background(), serviceContainer.NewScope(), serviceContainer)
    if _, _, err := userService.Update(runtimeInstance, user.Id, "admin", "changed", []string{entity.RoleAdmin}); nil == err {
        t.Fatal("expected duplicate username refusal")
    }
    stored, _, err := userService.userRepository.FindById(context.Background(), user.Id)
    if nil != err || original.Username != stored.Username || original.Password != stored.Password || false == reflect.DeepEqual(original.Roles, stored.Roles) {
        t.Fatalf("refused update changed the stored user: %v", err)
    }
}
