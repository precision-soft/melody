...
- example: reading a product, a category or a currency by an identifier the cache-key grammar refuses answers not-found instead of failing. The three readers handed the caller's spelling straight into a cache key, and both backends refuse a key carrying a space — measured on the running stack, an authenticated read of an id with an interior space answered 500 where the same request answers 404 on the sibling major, which has carried the guard on these three doors all along; the user reader already carried it here
...
- example: a credential refused because the STORED value is not a bcrypt digest now spends the comparison bcrypt skipped. bcrypt refuses such a value on its prefix, before deriving any key, so the refusal was four orders of magnitude cheaper than a real comparison — measured in package, 228ns against 51.86ms, a ratio of 227,477x, and on the wire through the load balancer 2.5ms against the 78ms an absent username pays. Response time therefore named every account whose column holds a value this door cannot use — truncated, edited by hand, written by something that is not this application — and those are exactly the accounts it will refuse whatever is typed, which is the existence oracle `DummyPasswordMatch` exists to close, inverted
