 ...
 *
 * URL generation goes through the RouteGenerator (melody-routes.ts): the server
 * injects the route manifest as window.melodyRoutes on each page, and every link,
 * form action and fetch target is built from a route name instead of a hardcoded
 * path. The manifest is assembled by the example itself, in url.RoutesJsonFromContainer,
 * out of the named routes the route registry holds.
 */
