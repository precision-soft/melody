...
/* registerCatalogJournalService wires the journal against whatever the environment gave the example; without a database the service is registered with nothing behind it, so the writes still succeed and the report shows a journal of zero. */
