...
import (
    ...
    exampleurl "github.com/precision-soft/melody/v2/.example/url"
...
func Html(runtimeInstance melodyruntimecontract.Runtime, request melodyhttpcontract.Request, statusCode int, fileName string) melodyhttpcontract.Response {
    ...
    /* the manifest sits inside a single-quoted JS string literal, so a backslash or single quote in the JSON is escaped for that context, backslash first; json.Marshal already escapes < > & and the line separators */
