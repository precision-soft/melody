...
/* canonicalizeAccessControlPath normalizes the path for rule lookup. HTTP callers must reject non-canonical spellings before routing or authorization: folding dot segments or surrounding whitespace can select a more permissive rule than the raw path. The HTTP kernel enforces this boundary; direct matcher callers must enforce the same agreement with their router. */
