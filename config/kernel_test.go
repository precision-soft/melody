...
/* the log level decides what every later record is compared against, so a name the logger does not know is refused instead of falling back: a deployment that misspells it would otherwise run at whatever level the fallback picked, and find out when an incident is missing from the log */
