...
# one line, a block continued across a CRLF line end and the repair vocabulary each count once, a word that only
# carries a phrase inside it counts nothing, and a file that ends inside a comment is read ahead of the negative one,
# so a state carried from one file into the next counts the negative file's raw string as history.
...
if [[ "${COMMENT_CONTROL_DIRECTORY_STRING}/positive.go"$'\t'"14" != "${COMMENT_CONTROL_OUTPUT_STRING}" ]]; then
    fail "the history comment control failed: expected the planted file alone with 14, read [${COMMENT_CONTROL_OUTPUT_STRING}] — no verdict over the tree is possible"
fi
