...
/* the stateless HMAC firewall guards /internal (config/security.go) and /internal/whoami/ echoes the service
principal it authenticated together with the actor the caller propagated (handler/internalauth/whoami_handler.go).
The route is POST-only and the signed method/path must match the request exactly, so every call below uses POST and
the same path the envelope was minted for. */
const internalAuthRoute = "/internal/whoami/"

/* internalAuthHeaderName is DefaultHmacHeaderName; internal:sign prints it above the envelope, and the section
reads that printed name rather than assuming it, so a rename in the framework surfaces here as a mismatch instead
of as a silent 401. */
...
/* runInternalAuthCheck exercises the cross-app HMAC scheme ACROSS A REAL SERVICE BOUNDARY: the envelope is minted
by the example's internal:sign command in one process and verified by the running application in another, over
http, through the whole request pipeline. The in-process hmac.go section stays exactly as it is — it drives the
token source directly against a live redis nonce guard, which is a different property (a shared, multi-instance
guard) and is not reachable through this route, whose firewall is configured without one.

Every negative mints its OWN envelope. Reusing one would make each negative pass for the wrong reason: the second
presentation of any envelope is refused as a replay, so a broken signature check would still look correct. */
...
/* assertInternalAuthAcceptedAndReplayed covers the positive and the replay in one place because they must share
one envelope: the replay negative is only meaningful for an envelope that was ACCEPTED first. */
func assertInternalAuthAcceptedAndReplayed(client *liveExampleClient) {
    ...
    /* the propagated actor is the whole point of the scheme: without it the callee knows only which SERVICE called
       and can neither authorize nor audit on behalf of the human who started the chain */
    ...
    /* the same envelope presented a second time must be refused: its nonce was consumed by the accepted call
       above, so a captured header on the wire buys an attacker exactly one nothing */
    ...
    /* a stale-envelope negative is deliberately NOT asserted here: the envelope's lifetime is
       exampleHmacEnvelopeLifetime (30s) and internal:sign has no flag to backdate one, so the only way to observe
       expiry over http would be to sleep 31 seconds inside the run. The expiry check is covered by the framework's
       own unit tests, which can move the clock. */
}
...
/* assertInternalAuthBodyMismatch signs one body and sends a different one. The envelope carries a hash of the body
it was minted for, so this is the assertion that a valid envelope cannot be lifted onto a rewritten request — the
exact attack a man-in-the-middle attempts against a signed api call. */
...
/* assertInternalAuthQueryMismatch signs a query string and sends another. The query is signed separately from the
path, so an envelope minted for ?tenant=acme must not authorize ?tenant=victim — without this bind, a signed call
can be redirected onto another tenant's data while every other check still passes. */
...
/* mintInternalAuthEnvelope runs internal:sign and returns the envelope together with how long the mint took. The
default path the command signs is internalAuthRoute, so a caller that passes no --path gets an envelope bound to the
route the section calls. The printed HEADER NAME is verified against the constant here rather than ignored: the
command prints it precisely so a client does not have to guess, and a divergence would otherwise surface as an
unexplained 401 in every assertion below. */
