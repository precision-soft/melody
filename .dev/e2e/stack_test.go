...
func TestStackScript_V2SecretNegativeRequiresANonEmptyEntry(t *testing.T) {
    ...
    sectionIndex := strings.Index(script, "check_section_start \"V2 DEBUG COMMANDS\"")
    if -1 == sectionIndex {
        t.Fatal("could not locate the v2 debug-commands section")
    }

    endIndex := strings.Index(script[sectionIndex:], "check_section_start \"V2 COMMAND OUTPUT ENVELOPE\"")
    if -1 == endIndex {
        t.Fatal("could not locate the end of the v2 debug-commands section")
    }

    region := script[sectionIndex : sectionIndex+endIndex]

    guardIndex := strings.Index(region, "[[ \"\" = \"${V2_API_TOKEN_ENTRY_STRING}\" ]]")
    negativeIndex := strings.Index(region, "grep -q 'example-api-token'")
