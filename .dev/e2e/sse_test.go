...
/* The frame parser is the riskiest new logic in the harness: every server-sent-event assertion reads its verdict
out of it, and a parser that quietly mis-split a frame would turn the id-sanitisation and topic-isolation negatives
into assertions that can never fail. These tests pin the grammar it implements. */
...
/* the preamble assertion turns on this distinction: a comment-only frame and a frame that merely carries no data
are different things on the wire, and treating the first as an event would let a stream that never flushed its
preamble pass the ordering assertion. */
...
/* a frame that carries a comment AND fields is an event, not a comment: the keepalive comment may share a frame
with a real event, and stepping over it as if the whole frame were a comment would drop the event. */
...
/* the writer emits one data: line per line of the payload, so a multi-line payload has to be rejoined — a parser
that kept only the last line would silently truncate every event carrying a newline. */
...
/* only ONE leading space is stripped after the colon, and only the FIRST colon separates the name from the value:
a payload that is itself json ("data: {\"a\":1}") must survive intact, and so must a deliberate leading space. */
...
/* a line with no colon is a field with an empty value, never a comment: reporting it as a comment would make a
malformed frame look like a keepalive and be silently stepped over. */
...
/* the blank line is the frame terminator, so two events in one stream must come out as two frames — this is the
exact property the id-sanitisation negative asserts the ABSENCE of, and a reader that merged them would make that
...
/* CRLF line endings must parse identically: a proxy or a client library that normalises them would otherwise leave
a trailing carriage return inside every value and every equality assertion in the section would fail. */
...
/* a closed body must report "no frame" rather than block until the budget expires: the assertions distinguish a
frame that did not arrive from a stream that ended, and only the second one means the application closed on us. */
