...
/* @info the exception event carries the failure beside the request, and its response starts empty: a listener sets one to answer the failure, and the kernel reads back whether any listener did. An event that reported a response it never received would make the kernel serve nil. */
