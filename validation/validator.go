...
/* validationJsonFieldName yields the name a payload spells the field with, and whether encoding/json omits it outright, whose zero value is then never validated (a field literally named "-" is spelled "-," and stays validated). */
...
/* promotesValidationTimeCodec reports a struct a payload can only spell as an RFC 3339 string, which populates nothing else in it, so its constraints are not enforced. It asks both halves: the promoted marshaler decides the wire form, the unmarshaler what a body populates. */
...
/* promotesValidationTimeEncoding reports a struct whose promoted json codec is time.Time's, the encoding half alone. */
...
/* promotedValidationMarshalerOrigin reports which type declares the MarshalJSON in the value method set and at what embedding depth, the shallowest winning and a tie promoting nothing. Unresolved (false) keeps the struct walked, the direction that keeps constraints enforced. */
