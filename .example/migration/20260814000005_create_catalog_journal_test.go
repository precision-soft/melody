...
func TestUpCreateCatalogJournalCreatesTheTableTolerantlyInPostgresDialect(t *testing.T) {
    ...
    if false == strings.Contains(queries[0], "id BIGINT GENERATED BY DEFAULT AS IDENTITY") {
        t.Fatalf("expected the journal identifier to stay database-assigned, got %q", queries[0])
    }
    if true == strings.Contains(queries[0], "`") {
        t.Fatalf("expected postgres identifiers without backticks, got %q", queries[0])
    }
}
