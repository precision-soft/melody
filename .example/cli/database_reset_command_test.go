...
/* an unwired journal is not an error on this major: the journal is a switch of its own, and the plan says
   so rather than the command refusing over a database this environment never asked for. */
func TestDatabaseResetCommandSaysWhenTheJournalIsNotWired(t *testing.T) {
    serviceContainer := newResetCommandContainer(t)
