...
/* The two implementations share one validation, so a write the in-memory catalogue refuses is refused by the database-backed one with the same words. The validation itself is pinned field by field in product_repository_test.go, beside the guard; what this asserts is that the door reaches it at all. */
